...
test("creates one queued job with deterministic lifecycle metadata", async () => {
  ...
  const boundary = createRefreshJobBoundary({ client: database, lifecycleVersion: "v1", now: () => "2026-08-28T00:00:00.000Z" });
...
test("rejects a duplicate active job before insert", async () => {
  ...
  const boundary = createRefreshJobBoundary({ client: database, lifecycleVersion: "v1" });
...
test("run owns queued-running-completed orchestration and returns work result", async () => {
  const database = client();
  const boundary = createRefreshJobBoundary({ client: database, lifecycleVersion: "v1", now: () => "now" });
...
test("fails closed for invalid composition dependencies", () => {
  assert.throws(() => createRefreshJobBoundary(), /database client/);
  assert.throws(() => createRefreshJobBoundary({ client: { from() {} } }), /lifecycleVersion/);
  assert.throws(() => createRefreshJobBoundary({ client: { from() {} }, lifecycleVersion: "v1", now: null }), /now must be a function/);
});
