...
test('emits only allowlisted failed gate codes without production counts', () => {
  assert.deepEqual(diagnostic.buildReport(rows()), { operation: 'e2_t6_rls_v3', status: 'DIAGNOSTIC_COMPLETE', checkedGates: 19, failedGateCodes: ['E2_T6_RESIDUE'], productionCountsExposed: false });
});
test('rejects missing, extra, duplicate, malformed, and all-pass results', () => {
  ...
  assert.throws(() => diagnostic.buildReport(rows('missing')), /at least one/);
});
