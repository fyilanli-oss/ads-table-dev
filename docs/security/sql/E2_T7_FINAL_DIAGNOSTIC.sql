-- E2-T7 corrective diagnostic. Read-only; returns gate classifications without counts or identities.
...
with expected(connected_rows,encrypted_rows) as (values ((-1)::bigint,(-1)::bigint)), policy_state as (
  ...
  union all select 'DATASET_V1_BASELINE',(select count(*) from public.performance_dataset_rows),(-1)::bigint,'eq'
