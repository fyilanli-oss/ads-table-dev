...
test("E10-T6-C advances through runtime composition and the embedded Platforms surface", () => {
  assert.equal(contract.status, "REPOSITORY_READY_HUMAN_SMOKE_PENDING");
  ...
  assert.deepEqual(contract.completed_repository_slices, [
    ...
    "embedded_provider_start_and_callback_adapter", "runtime_composition", "embedded_platforms_connect_surface",
  ]);
  assert.equal(contract.next_repository_slice, "human_development_oauth_smoke");
});
