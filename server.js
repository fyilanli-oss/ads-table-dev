...
app.get("/auth/organic",async(req,res)=>{
  try{
    ...
    req.session.organicOAuthStartedAt=new Date().toISOString();
...
async function googleAdsSearch(userId,customerId,query,loginCustomerId){const token=await getFreshGoogleAccessToken(userId);const clean=normalizeCustomerId(customerId);console.log("[METRICS REQUEST]\n");console.log(`loginCustomerId: ${loginCustomerId?normalizeCustomerId(loginCustomerId):""}\n`);console.log(`customerId: ${clean}`);if(query)console.log(`\nGAQL:\n${query}`);const r=await fetch(`https://googleads.googleapis.com/${GOOGLE_ADS_API_VERSION}/customers/${clean}/googleAds:search`,{method:"POST",headers:googleHeaders(token,loginCustomerId),body:JSON.stringify({query})});const data=await r.json();if(!r.ok){const err=new Error(JSON.stringify(data));err.status=r.status;throw err}return data}
...
app.get("/api/google/insights",async(req,res)=>{try{const user=await requireUser(req,res);if(!user)return;const customerId=req.query.customerId||req.query.customer_id;if(!customerId)return res.status(400).json({error:"Missing customerId"});const level=["campaign","adgroup","ad"].includes(String(req.query.level||"campaign"))?String(req.query.level||"campaign"):"campaign";const dateRange=String(req.query.date_range||req.query.dateRange||"last_7d");const loginCustomerId=req.query.loginCustomerId||req.query.login_customer_id||"";const query=googleQuery(level,dateRange);const insightCustomerIds=await resolveGoogleInsightCustomerIds(user.id,customerId,loginCustomerId);const performanceRows=[];const breakdownRows=[];const lpvRows=[];const breakdownErrors=[];const lpvErrors=[];let fieldMask=null,conversionFieldMask=null,landingPageViewFieldMask=null,requestId=null,nextPageToken=null;for(const insightCustomerId of insightCustomerIds){const data=await googleAdsSearch(user.id,insightCustomerId,query,loginCustomerId);performanceRows.push(...(data.results||[]));fieldMask=fieldMask||data.fieldMask||null;requestId=requestId||data.requestId||null;nextPageToken=nextPageToken||data.nextPageToken||null;try{const breakdownData=await googleAdsSearch(user.id,insightCustomerId,googleConversionBreakdownQuery(level,dateRange),loginCustomerId);breakdownRows.push(...(breakdownData.results||[]));conversionFieldMask=conversionFieldMask||breakdownData.fieldMask||null}catch(err){breakdownErrors.push(err.message)}try{const lpvData=await googleAdsSearch(user.id,insightCustomerId,googleLandingPageViewQuery(level,dateRange),loginCustomerId);lpvRows.push(...(lpvData.results||[]));landingPageViewFieldMask=landingPageViewFieldMask||lpvData.fieldMask||null}catch(err){lpvErrors.push(err.message)}}const withConversions=mergeGoogleConversionActions(performanceRows,breakdownRows,level);const mergedRows=mergeGoogleLandingPageViews(withConversions,lpvRows,level);res.json({platform:"Google",level,customerId:normalizeCustomerId(customerId),loginCustomerId:loginCustomerId?normalizeCustomerId(loginCustomerId):null,date_range:dateRange,rows:mergedRows.map(r=>normalizeGoogleInsight(r,level)),rawCount:mergedRows.length,conversionBreakdownCount:breakdownRows.length,conversionBreakdownError:breakdownErrors.length?breakdownErrors.join(" | "):null,landingPageViewCount:lpvRows.length,landingPageViewError:lpvErrors.length?lpvErrors.join(" | "):null,fieldMask,conversionFieldMask,landingPageViewFieldMask,requestId,nextPageToken})}catch(e){res.status(e.status||500).json({error:e.message})}});
