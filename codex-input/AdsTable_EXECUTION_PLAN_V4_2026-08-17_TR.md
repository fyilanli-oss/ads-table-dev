...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-D2 Klaviyo live PASS; R6-D3-A+B Meta contract/token repository Done; R6-D3-C repository preflight PASS, production merchant acceptance pending` — Meta legacy kaydı korunur fakat expired token'ı kullanılmaz. Temiz embedded OAuth için short-lived → long-lived exchange ile app/`ads_read`/expiry debugger doğrulaması repository düzeyinde uygulanmıştır. Mevcut R7-A2 account-selection runtime'ı yeniden kullanılacaktır: callback yalnız `pending_account_selection`; server'ın kaydetme anında yeniden doğruladığı 1–3 hesap seçilmeden `Connected` değildir. Production deployment/OAuth, Dataset V2, schedule/backfill ve E4 yeniden geliştirme yapılmadı. |
...
- **R6-D iş aynası — In progress / R6-D1 Done; R6-D2 Klaviyo live PASS; R6-D3-A+B repository Done; R6-D3-C repository preflight PASS / production merchant acceptance gate:** R6-D tek bir belirsiz production açılışı değildir. Sabit sıra: **R6-D1** ortak fail-closed kabul koşucusu; **R6-D2** Klaviyo workspace canlı kabulü; **R6-D3** Meta workspace canlı kabulü; **R6-D4** Google Ads workspace canlı kabulü; **R6-D5** provider bazlı kontrollü aktivasyon kararı. Klaviyo provider-doğrulanmış boş sonuçla PASS olmuştur; bu sonuç Meta veya Google Ads'i aktive etmez. Meta legacy/token kararı, long-lived token doğrulaması ve mevcut R7-A2 1–3 hesap seçim runtime'ının repository kabulü tamamlanmıştır; deployment, gerçek OAuth ve merchant acceptance ayrı kapıdır. TikTok/Pinterest parked kalır. R6-D bütün provider kapıları tamamlanmadan `Done` olmaz; R7-B kapalı kalır.
...
- **R6-D3-C Meta 1–3 hesap seçimi — Repository preflight PASS / production acceptance runner prepared locally / merchant acceptance pending:** R7-A2'de kurulmuş canonical seçim hattı eski `server.js + dashboard.html` referansıyla yeniden karşılaştırıldı ve yeni runtime yazılmasına gerek olmadığı doğrulandı. Shopify session-bound liste/seçim rotaları pending encrypted grant ile Meta `/me/adaccounts` listesini server-side okur; browser yalnız ID gönderir ve save anında provider listesi yeniden alınır. Sıfır, üçten fazla, duplicate veya listede olmayan seçim reddedilir. Yalnız doğrulanmış 1–3 hesap optimistic connection version ile `selected_accounts` alanına yazılır; ilk hesap `active_account_id` compatibility alanıdır ve ancak bu update sonrasında durum `connected` olur. Shopify-native `s-choice-list`/`s-modal` kullanımı güncel resmi component sözleşmesiyle uyumludur. Supabase validated cardinality constraint'i ve browser grant `0` sınırı korunur. Canlı kabul için tek-geçişli analist koşucusu ile salt-okunur preflight/postcheck hazırlanmıştır; legacy Meta kaydı korunacak, Dataset V2/schedule/job `0` kalacaktır. Bu hazırlık production'a dağıtılmamış veya çalıştırılmamıştır; gerçek merchant Connect→OAuth→modal→1–3 seçim→Save→reload kanıtı olmadan PASS verilmeyecektir. Production deployment, canlı Meta OAuth/provider teması, Dataset V2, schedule/backfill, Disconnect/revoke veya E4 yeniden geliştirme yapılmadı. Sözleşme `contracts/r6d3c-meta-account-selection-acceptance-v1.json`, analist kaydı `docs/R6D3C_META_ACCOUNT_SELECTION_ACCEPTANCE.md`, koşucu `docs/R6D3C_META_PRODUCTION_MERCHANT_ACCEPTANCE_RUNBOOK.md` içindedir. Sıradaki kapı ayrı açık onaylı production merchant acceptance'tır.
