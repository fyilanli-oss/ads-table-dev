...
- **R6-D2-C6 kontrollü Dataset V2 canlı kabulü — C6-A gate prepared / production write not executed:** Tamamlanmış Klaviyo runner, mapper, workspace canonical write boundary ve Supabase repository yalnız internal acceptance query yüzeyinde session-bound ve exact action-time confirmation ile birleştirilir. Provider gerçek non-empty döndürürse yalnız gerçek canonical satırlar upsert edilir; verified-empty döndürürse sıfır satır yazılır. Otomatik retry, cron veya production activation yoktur. Production write ayrı açık onay almadan çalıştırılmaz.
