...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-D2 Klaviyo live PASS; R6-D3 Meta full lifecycle live PASS; R6-D4-E Google Ads controlled Dataset V2 verified-empty live PASS / R6-D4-F Disconnect brief next` — Google Sheets ve GA4 parked kalır. Üç hesap için tek kontrollü kabul `attempted: 0 / persisted: 0 / verified empty: true` vermiş; Supabase son kontrolü canonical bağlantı ve üç hesabı doğrularken Dataset V2 toplam/Google/sentetik satırlarını `0/0/0` göstermiştir. Schedule, backfill ve production aktivasyonu kapalıdır. |
...
- **R6-D iş aynası — In progress / R6-D1 Done; R6-D2 Klaviyo live PASS; R6-D3 Meta full lifecycle live PASS; R6-D4-E Google Ads controlled Dataset V2 verified-empty live PASS / R6-D4-F Disconnect brief next:** R6-D tek bir belirsiz production açılışı değildir. Sabit sıra: **R6-D1** ortak fail-closed kabul koşucusu; **R6-D2** Klaviyo workspace tam yaşam döngüsü; **R6-D3** Meta workspace tam yaşam döngüsü; **R6-D4-A0** eski Google Sheets Dataset V1 export utility'sinin güvenli parkı; **R6-D4** Google Ads workspace tam yaşam döngüsü; **R6-D5** provider bazlı kontrollü aktivasyon kararı. Meta Connect → OAuth → verified account selection → data acceptance → Disconnect → temiz Reconnect zinciri canlıda tamamlanmıştır. Google Ads bağlantı ve üç doğrulanmış hesap seçimi reload kalıcılığıyla canlıda kanıtlanmıştır. E5 motorunu workspace authority ile çağıran salt-okunur preflight üç hesapta canlıda PASS olmuştur; Standard/PMax sonucu doğrulanmış boş, Time/FX başarılı ve Dataset V2 `0` olarak Supabase son kontrolüyle doğrulanmıştır. Google Sheets/GA4 ile TikTok/Pinterest parked kalır. R6-D bütün provider kapıları tamamlanmadan `Done` olmaz; R7-B yalnız son ortak tutarlılık kapısıdır.
