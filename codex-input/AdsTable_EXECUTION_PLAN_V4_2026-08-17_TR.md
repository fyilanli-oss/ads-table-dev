...
**Durum:** `In progress — E10-T5 product freeze Done (C1–C7; C5-B Deferred); E10-T6-A Done/PASS; E10-T6-B managed-installation runtime ve remote Supabase migration PASS; Shopify env 4/4 ve keyring girdileri 1/2 görünür, smoke encryption key map process injection bekliyor; every production contact gated`
...
**Durum:** `In progress / Blocked — configured secret not injected into the smoke process`.
...
- **2026-09-10 smoke sonucu:** Dört Shopify girdisi değerleri ve uzunlukları gösterilmeden 4/4 görünür ve Development Store domain'i canonical doğrulanmıştır; remote Supabase salt-okunur güvenlik/migration postcheck yeniden PASS'tir. Tekrarlanabilir redacted preflight somut olarak çalıştırıldı. Keyring girdileri 1/2 görünürdür: active key kimliği process'tedir, encryption key map değildir; preflight/runtime fail-closed kalmıştır. Vercel deployment/CI PASS'tir; operator App URL route probe ağ geçidinde engellenmiştir ve bu sonuç uygulama arızası kanıtı değildir. Credentialsız Development Store reachability probe'u HTTP 4xx yanıtıyla ağ temasını doğrulamış, authentication/install kabulü üretmemiştir. Gerçek session/ID token, token exchange, Admin Shop identity, binding, encrypted persistence ve reopen/reinstall çalıştırılmamıştır; production teması olmamıştır. E10-T6-B `In progress / Blocked`, E10-T6-C kapalıdır. Redacted evidence `artifacts/e10-shopify/e10-t6b-development-store-smoke.json` içindedir.
