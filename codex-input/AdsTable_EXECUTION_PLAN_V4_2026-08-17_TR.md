...
**Durum:** `Verification` — PR #117 merge edildi ve `main` kontrolleri başarılıdır. İlk post-merge deneme `Manual refresh missing Google login customer id` ile job açılmadan durdu. PR #118 hierarchy keşfini ekledi; ilk reconnect kontrolü root hierarchy sorgusunda root'un kendisini `login-customer-id` header'ı olarak da gönderirken `Google Ads request failed` verdi ve hata URL'si `Close` sonrasında aynı modalı yeniden açtı. Provider'ın redacted hatası kesin alt kodu taşımadığı için corrective revizyon resmi doğrudan-root çağrı modeline döner: root sorgusunda login header'ını kaldırır, root'u yalnız child ad account istekleri için login context olarak korur ve hata halinde reconnect URL'sini tüketir. Canlı tekrar kontrolü yapılmadan kök neden `Done` sayılmaz; Meta ile aynı completed/no-error kabul sonucu doğrulanmadan E6 başlamaz.
