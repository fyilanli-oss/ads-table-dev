-- Read-only, redacted E1-T6E post-operation acceptance.
with connection_facts as (
  select pc.platform, pc.connected,
...
), checks(check_name, platform, row_count) as (
  ...
  union all select 'platform_connected_connections', platform, count(*) filter (where connected)::bigint from connection_facts group by platform
  union all select 'platform_encrypted_connections', platform, count(*) filter (where has_encrypted_row)::bigint from connection_facts group by platform
