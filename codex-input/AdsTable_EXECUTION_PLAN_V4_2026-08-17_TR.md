...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-D2 Klaviyo live PASS; R6-D3-E Meta data live PASS; R6-D3-F repository implementation PASS, production acceptance next` — Meta legacy kaydı korunmuş ve expired token'ı kullanılmamıştır. Temiz embedded OAuth, bir provider-doğrulanmış hesap seçimi, tamamlanmış E4 motoru ve kontrollü verified-empty Dataset V2 kabulü production'da doğrulanmıştır. Meta'nın bağımsız Disconnect kodu ve repository testleri hazırdır; production Disconnect → temiz Reconnect doğrulanmadan R6-D3 ve Google Ads R6-D4 açılmaz. |
...
- **R6-D3-F Meta bağımsız Disconnect yaşam döngüsü — Repository implementation PASS / production acceptance pending:** Eski `server.js + dashboard.html` referansındaki revoke-first davranış embedded yapıya taşındı. Meta kartında Shopify-native warning modalı, doğrulanmış Shopify session'a bağlı endpoint ve optimistic canonical `disconnectMeta` cleanup operasyonu vardır. Provider revoke başarısızsa bağlantı ve credential değişmez; başarılıysa yalnız Meta token/scope/expiry ve seçilmiş hesap alanları temizlenir. Dataset V1/V2, legacy Meta geçmişi, reporting currency, Klaviyo ve gelecekteki Google bağlantısı korunur. Mevcut şema yeterlidir; migration yapılmadı. 55 odaklı repository testi PASS verdi. Production deployment, canlı Disconnect ve temiz Reconnect henüz doğrulanmadı; provider teması veya Supabase production mutation yapılmadı. Sözleşme `contracts/r6d3f-meta-independent-disconnect-v1.json`, analist kaydı `docs/R6D3F_META_INDEPENDENT_DISCONNECT.md` içindedir. Sıradaki kapı production deployment ve merchant acceptance'tır; Google Ads henüz başlamaz.
