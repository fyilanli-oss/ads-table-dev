...
app.get("/api/audit/organic/ga4-source-medium",async(req,res)=>{
  ...
  try{
    ...
    propertyId=normalizePlatformAccountId(
      conn.account_id||
      conn.metadata?.selectedGa4Property?.property_id||
      conn.metadata?.selectedGa4Property?.platform_account_id||
      String(conn.metadata?.selectedGa4Property?.property_resource_name||"").replace(/^properties\//,"")
    );
