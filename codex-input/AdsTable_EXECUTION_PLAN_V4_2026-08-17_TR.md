...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-D2 Klaviyo live PASS; R6-D3-C Meta connection/account-selection live PASS; Meta data runtime analysis next` — Meta legacy kaydı korunmuş ve expired token'ı kullanılmamıştır. Temiz embedded OAuth; short-lived → long-lived exchange, app/`ads_read`/expiry debugger doğrulaması ve mevcut R7-A2 seçim runtime'ı ile production'da tamamlanmıştır. Bir provider-doğrulanmış hesap seçilmiş, `Connected · 1 account` reload sonrasında korunmuş; legacy kayıt `1 → 1`, Dataset V2/schedule/job `0` kalmıştır. Meta performans okuma, Time/FX ve Dataset V2 sonucu ayrıca kanıtlanmadan R6-D3 `Done` değildir. |
