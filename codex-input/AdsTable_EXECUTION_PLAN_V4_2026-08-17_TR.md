...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-D2 Klaviyo live PASS; R6-D3-A+B Meta contract/token repository Done; R6-D3-C account-selection acceptance gate` — Meta legacy kaydı korunur fakat expired token'ı kullanılmaz. Temiz embedded OAuth için short-lived → long-lived exchange ile app/`ads_read`/expiry debugger doğrulaması repository düzeyinde uygulanmıştır. Callback yalnız `pending_account_selection`; server'ın yeniden doğruladığı 1–3 hesap seçilmeden `Connected` değildir. Production deployment/OAuth, Dataset V2, schedule/backfill ve E4 yeniden geliştirme yapılmadı. |
