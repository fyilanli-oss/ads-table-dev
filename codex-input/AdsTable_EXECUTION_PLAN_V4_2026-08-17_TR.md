...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-A+B+C+D1 Done; R6-D2 Klaviyo live PASS; verified empty / rows 0; R6-D3 Meta approval gate` — C6-D token lifecycle production'a alındı. Zorunlu salt-okunur preflight Account, Campaign, Flow, Time ve FX kapılarında PASS verdi. Tek kontrollü C6 isteği provider-doğrulanmış boş sonuçla `attempted 0 / persisted 0 / verified empty true` döndürdü; sentetik satır yazılmadı. Supabase postcheck Dataset V2 `0`, connected canonical Klaviyo `1`, access expiry `1`, encrypted refresh `1` ve metric binding `1` doğruladı. Tamamlanmış E4/E5/E7 ve satış metriği politikası değiştirilmedi. |
