...
- **E1-T6 — `In progress` — Token protection:** E1-T6A vault ve E1-T6B canlı schema/RLS/grant kabulü tamamlandı. E1-T6C encryption ve legacy-read flagleri Production'da `true` durumundadır. E1-T6D cleanup sonrası 7 connection'ın tamamı auth-valid, encrypted ve Connected'dır. Meta, Google, TikTok, Klaviyo ve Organic refresh job'ları tamamlandı; 5 yeni dashboard snapshot ve Dataset V1'de toplam 16 satır (Google 3, Klaviyo 1, Meta 3, Organic 6, TikTok 3) oluştu, Dataset V2'ye beklenmedik runtime write olmadı. Böylece encrypted runtime/provider read kabulü tamamlandı. E1-T6E yalnız preflight/nulling/acceptance artefaktlarını hazırlar; production nulling henüz yapılmadı.
...
**Gerçekleşen (E1-T6C — activated):** Encrypted store merkezi `getConnection` hydration yoluna bağlıdır; authenticated envelope decrypt hatası legacy'ye düşmez. Production'da `PROVIDER_TOKEN_ENCRYPTION_ENABLED=true` ve `PROVIDER_TOKEN_LEGACY_READ_ENABLED=true` durumundadır. Yedi mevcut bağlantının tamamı Connected görünmüş; Meta, Google, TikTok, Klaviyo ve Organic refresh job'ları tamamlanarak encrypted runtime/provider read zinciri kabul edilmiştir.

**Gerçekleşen (E1-T6D — completed activation evidence):** Production dry-run `9 eligible`, controlled write-mode `7 written / 2 auth-orphan failure` verdi. Aynı kullanıcıya ait Meta/Pinterest orphan bağlantıları guarded operational cleanup ile silindi. Post-cleanup acceptance `7 connected / 7 encrypted / 0 auth-orphan / 0 connected-without-encrypted-token` sonucunu verdi. Ardından beş provider refresh job'ı tamamlandı, 5 snapshot üretildi, Dataset V1'e 16 satır yayımlandı ve Dataset V2'ye beklenmedik write olmadı.

**E1-T6E — `artefact ready / operation pending`:** Bu PR yalnız salt-okuma plaintext preflight ve acceptance ile fail-closed operational nulling SQL'ini hazırlar. Plaintext tokenlar Production'da henüz null yapılmadı; operational SQL çalıştırılmadı, legacy read kapatılmadı ve plaintext kolonlar drop edilmedi. E1-T6E `Done` değildir. Sonraki kapı salt-okuma preflight sonucunun ayrıca incelenmesi ve ayrıca onaylanmış manuel SQL Editor uygulamasıdır.
...
2. **E1-T6C — Dual path (activated):** Production encryption ve legacy-read flagleri `true`; encrypted runtime/provider read acceptance tamamlandı.
3. **E1-T6D — Backfill/rotation (accepted):** Dry-run ve controlled write sonrası auth-orphan cleanup tamamlandı; canlı kabul 7 connected, 7 encrypted, 0 orphan ve 0 missing encrypted sonucunu verdi.
4. **E1-T6E — Plaintext nulling (`artefact ready / operation pending`):** Ayrı preflight incelemesi ve onaylı manuel uygulama beklenir; legacy read kapatma ve kolon drop bu adımın kapsamında değildir.
...
**Evidence:** E1-T6B schema/RLS/grant acceptance tamamlandı. E1-T6D dry-run `9 eligible`, controlled write `7 written / 2 auth-orphan failure` verdi; onaylı orphan cleanup sonrasında kabul `7 connected / 7 encrypted / 0 orphan / 0 missing encrypted` oldu. Production encryption ve legacy-read flagleri `true` iken Meta, Google, TikTok, Klaviyo ve Organic refresh job'ları tamamlandı; 5 yeni dashboard snapshot oluştu, Dataset V1'e 16 satır yayımlandı ve Dataset V2'ye beklenmedik runtime write olmadı. E1-T6E artefaktları token, key, PII veya ham kullanıcı kimliği içermez; nulling SQL Production'da çalıştırılmadı ve ayrı preflight incelemesi ile manuel onay beklenmektedir.
