...
**C2I-V9 real-device Shopify-native acceptance — FAIL / corrective implemented:** 13 Eylül 2026 gerçek cihaz koşusunda uygulama Shopify Admin içinde açıldı, fakat App Home `SHOPIFY_INSTALL_PERSISTENCE_FAILED` ile durdu. App Home'un aynı install bootstrap yazımını art arda iki kez çalıştırdığı saptandı; akış tek bootstrap ve ardından ayrı session doğrulaması yapacak şekilde düzeltildi. Ekran görüntüsü ve referans kimliği repository'ye alınmadı. V9 ancak düzeltme deploy edilip App Home yeniden açıldığında tekrar değerlendirilebilir. OAuth, consent, callback, account discovery, provider API veya Production mutation yetkisi verilmez. Ayrıntılı sözleşme `contracts/shopify/e10-t6-c2i-v9-real-device-acceptance.json` ve `docs/E10_T6_C2I_V9_REAL_DEVICE_ACCEPTANCE.md` içindedir.
