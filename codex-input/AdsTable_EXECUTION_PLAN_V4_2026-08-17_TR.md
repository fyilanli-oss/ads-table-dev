...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-D2 Klaviyo live PASS; R6-D3 Meta live PASS; R6-D4 Google Ads analyst brief next` — Meta legacy kaydı korunmuş ve expired token'ı kullanılmamıştır. Temiz embedded OAuth, bir provider-doğrulanmış hesap seçimi ve tamamlanmış E4 motorunun workspace authority üzerinden salt-okunur çalışması production'da doğrulanmıştır. Açık onaylı kontrollü Dataset V2 kabulü provider-doğrulanmış boş sonuçla `attempted 0 / persisted 0 / synthetic 0` PASS vermiş; Supabase postcheck yanlış yazı olmadığını doğrulamıştır. Non-empty fiziksel UPSERT henüz gözlenmemiştir. R6-D bütünü Google Ads kabulü ve provider bazlı aktivasyon kararı tamamlanmadan `Done` değildir. |
...
- **R6-D iş aynası — In progress / R6-D1 Done; R6-D2 Klaviyo live PASS; R6-D3 Meta live PASS; R6-D4 Google Ads analyst brief next:** R6-D tek bir belirsiz production açılışı değildir. Sabit sıra: **R6-D1** ortak fail-closed kabul koşucusu; **R6-D2** Klaviyo workspace canlı kabulü; **R6-D3** Meta workspace canlı kabulü; **R6-D4** Google Ads workspace canlı kabulü; **R6-D5** provider bazlı kontrollü aktivasyon kararı. Klaviyo provider-doğrulanmış boş sonuçla PASS olmuştur; bu sonuç Meta veya Google Ads'i aktive etmez. Meta bağlantı/hesap seçimi, E4 Insights/Time/FX salt-okunur runtime ve açık onaylı kontrollü Dataset V2 kabulü production'da doğrulanmıştır. Kontrollü kabul gerçek provider'dan `0/0 verified empty` dönmüş, sentetik satır üretmemiş ve Supabase postcheck Dataset V2 Meta/schedule/job sayılarını `0` doğrulamıştır; non-empty fiziksel UPSERT henüz gözlenmemiştir. TikTok/Pinterest parked kalır. R6-D bütün provider kapıları tamamlanmadan `Done` olmaz; R7-B kapalı kalır.
