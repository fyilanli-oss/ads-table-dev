...
Each of the 35 cases has a distinct namespaced entity key and its own nested exception subtransaction containing a static `INSERT`. There is no dynamic SQL, `EXECUTE`, `format()`, SQL concatenation, retry, `ON CONFLICT`, `UPDATE`, `DELETE`, `TRUNCATE`, or ad hoc cleanup. Expected rejection rolls back the nested subtransaction. An unexpectedly accepted insert is recorded as failed and remains only until the outer mandatory rollback. `COMMIT` is forbidden.
