...
İlk aşama kanıtı `GOOGLE_CUSTOMER_METADATA` oldu. Kesin kök neden iki fonksiyon arasındaki çağrı şekli uyuşmazlığıdır: live modül search bağımlılığını `{userId, customerId, query, loginCustomerId}` nesnesiyle çağırırken root `googleAdsSearch` bu değerleri dört ayrı positional argüman olarak bekliyordu. Nesnenin tamamı `userId` yerine geçerek token store PostgreSQL UUID filtresine `[object Object]` olarak ulaşıyordu. Composition adapter nesneyi açıkça dört positional değere çevirir; böylece hata Dataset V2'den önce, gerçek oluştuğu sınırda giderilir.

İmza düzeltmesinden sonraki canlı job `GOOGLE_PROVIDER_ADAPTER` aşamasına ulaştı ve `Google search response must contain results[]` verdi. Google Ads Search cevabındaki `results` tekrarlanan bir ProtoJSON alanıdır; sorgu sıfır satır döndürdüğünde başarılı cevapta bu alan hiç bulunmayabilir. Runtime artık HTTP başarısı sonrasında eksik `results` alanını geçerli zero-row olarak kabul eder. Alan mevcut fakat dizi değilse cevap hâlâ fail-closed reddedilir; bu ayrım gerçek provider hatasını sessizce başarıya çevirmeden test hesabının boş sonucunu kabul eder.

## Canlı kabul

PR #125 merge ve Vercel dağıtımından sonra kullanıcı normal Google Refresh işlemini çalıştırdı. 2026-08-31 22:43:36 UTC'de açılan job `completed` durumuna geçti; hata mesajı ve `failure_stage` oluşmadı, kalıcı `google_v2_evidence` kaydedildi. Test hesabının boş provider sonucu sahte Dataset V2 satırı yaratmadan başarıyla tamamlandı. Bu sonuç Meta için kullanılan zero-row/no-error kabul modeliyle aynıdır ve E5-T7 ile E5 epic'ini kapatır.
