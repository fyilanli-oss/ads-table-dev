...
function buildEvidence(result) {
  ...
  assert(result.operation_code === 'e2_t5_rejection_v2', 'operation_code mismatch');
  ...
  return Object.freeze({
    evidence_version: 'e2-t5-rejection-v2', operation_code: 'e2_t5_rejection_v2', status: 'PASS',
...
function main(argv) {
  assert(argv.length === 1, 'usage: node scripts/e2-t5-v2-evidence.js <redacted-result.json>');
...
if (require.main === module) {
  ...
  catch (error) { process.stderr.write('E2-T5 V2 evidence validation failed\n'); process.exitCode = 1; }
}
