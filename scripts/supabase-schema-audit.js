...
const DATASET_V2_CONSTRAINTS = ["performance_dataset_rows_v2_pkey","performance_dataset_rows_v2_platform_chk","performance_dataset_rows_v2_traffic_type_chk","performance_dataset_rows_v2_source_system_chk","performance_dataset_rows_v2_channel_chk","performance_dataset_rows_v2_campaign_type_chk","performance_dataset_rows_v2_root_type_chk","performance_dataset_rows_v2_parent_type_chk","performance_dataset_rows_v2_entity_type_chk","performance_dataset_rows_v2_source_confidence_chk","performance_dataset_rows_v2_source_currency_chk","performance_dataset_rows_v2_target_currency_chk","performance_dataset_rows_v2_fx_rate_chk","performance_dataset_rows_v2_metric_support_object_chk","performance_dataset_rows_v2_raw_object_chk","performance_dataset_rows_v2_synthetic_chk","performance_dataset_rows_v2_source_semantics_chk","performance_dataset_rows_v2_hierarchy_chk","performance_dataset_rows_v2_metric_support_keys_chk","performance_dataset_rows_v2_metric_value_support_chk"];
...
function datasetV2Drifts(relation, policies) {
  ...
  if(relation.pkCount!==1||relation.fkCount!==1||relation.uniqueCount!==0) add("constraint-counts",JSON.stringify({pk:relation.pkCount,fk:relation.fkCount,unique:relation.uniqueCount}),"pk 1; fk 1; unique constraints 0");
  for(const constraint of relation.constraints||[]){const normalized=String(constraint.definition||"").toLowerCase().replace(/\s+/g," ");const missing=(DATASET_V2_CONSTRAINT_TOKENS[constraint.name]||[]).filter(token=>!normalized.includes(token));if(missing.length)add(`constraint-definition-${constraint.name}`,normalized,`required semantic tokens: ${missing.join(", ")}`);}
