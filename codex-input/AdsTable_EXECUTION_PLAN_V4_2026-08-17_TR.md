...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-A+B+C+D1 Done; R6-D2-C1+C2+C3+C4+C5 live PASS; R6-D2-C6-A controlled write gate prepared` — C5 production runtime HTTP `200` verdi; Account, Campaign, Flow, Time ve FX kontrolleri PASS, Dataset V2 write `0` olarak bağımsız UI/runtime/Supabase kanıtıyla doğrulandı. C6-A yalnız mevcut workspace runner, canonical boundary ve Supabase repository'yi session-bound, explicit-confirmation kabul kapısında birleştirir; production Dataset V2 write henüz çalıştırılmaz. C6 production write ve C7 postcheck ayrı açık onaylar olmadan tamamlanmış sayılmaz; tamamlanmış E4/E5/E7 yapıları yeniden geliştirilmez. |
...
- **R6-D2-C6 kontrollü Dataset V2 canlı kabulü — C6-A gate prepared / production write not executed:** Tamamlanmış Klaviyo runner, mapper, workspace canonical write boundary ve Supabase repository yalnız internal acceptance query yüzeyinde session-bound ve exact action-time confirmation ile birleştirilir. Yazmadan önce aynı workspace + Klaviyo account + kapalı provider tarihi için Dataset V2 satırı aranır; kayıt varsa tekrar çalışma provider temasından önce engellenir. Provider gerçek non-empty döndürürse yalnız gerçek canonical satırlar upsert edilir; verified-empty döndürürse sıfır satır yazılır. Otomatik retry, cron veya production activation yoktur. Production write ayrı açık onay almadan çalıştırılmaz.
