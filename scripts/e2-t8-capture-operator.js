...
function parseArgs(args) {
  ...
  if (args.length === 3 && args[0] === 'diagnose-ownership' && args[1] === '--confirm') return { action: 'diagnose-ownership', confirmation: args[2] };
...
async function main() {
  ...
  const report = args.action === 'preflight'
    ? await op.preflight({ repo, stateFile, confirmation: args.confirmation, client: client() })
    : args.action === 'diagnose-ownership'
      ? await op.diagnoseOwnership({ repo, stateFile, confirmation: args.confirmation, client: client() })
