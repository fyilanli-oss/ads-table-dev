...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-D2 Klaviyo live PASS; R6-D3 Meta full lifecycle live PASS; R6-D4-A0 Google Sheets park repository PASS / production gate` — Google Sheets'in eski Dataset V1 export hattı için yeni OAuth, token refresh, manuel/otomatik sync ve disconnect mutation repository'de fail-closed kapatılmıştır; tarihsel bağlantı, encrypted credential ve spreadsheet korunur. Bu karar henüz production'a dağıtılmamıştır. Google Ads yaşam döngüsü R6-D4-A0 production doğrulamasından sonra başlar. |
...
- **R6-D4-A0 Google Sheets park kararı — Repository PASS / production deployment gate:** Google Sheets mevcut yapıda provider data source değil, eski `performance_dataset_rows` Dataset V1 içeriğini spreadsheet'e taşıyan standalone export utility'sidir. Salt-okunur canlı envanter tarihsel connected kayıt, encrypted access/refresh credential, bağlı spreadsheet, açık auto-sync ve geçmiş başarılı sync bulunduğunu doğrulamıştır; hiçbir token/spreadsheet kimliği kanıta alınmamıştır. Onaylanan A0 ile repository'de yeni OAuth ve callback exchange, token refresh, manuel/otomatik sync, Dataset V1/V2 export ve disconnect mutation fail-closed kapatılmış; status yüzeyi `Parked` olmuştur. Mevcut spreadsheet, tarihsel connection ve encrypted credential değiştirilmez; Google grant revoke edilmez. Contract `contracts/r6d4a0-google-sheets-park-v1.json`, analist kaydı `docs/R6D4A0_GOOGLE_SHEETS_PARK.md` içindedir. Bu PASS yalnız repository uygulamasıdır: production deployment, post-deploy salt-okunur doğrulama, Supabase mutation veya Google provider çağrısı yapılmamıştır. R6-D4-A Google Ads ancak A0 production kapısı kapandıktan sonra başlar.
