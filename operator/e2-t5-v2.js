...
const ARTIFACTS = Object.freeze([...Object.values(SQL), 'scripts/e2-t5-v2-evidence.js', 'artifacts/dataset-v2-acceptance/e2-t5-rejection/rejection-matrix.json']);
...
function validatePreflight(rows) {
  if (!Array.isArray(rows) || rows.length !== 18) throw new Error('Preflight must return exactly 18 gates');
  if (rows.some((row) => !row || row.passed !== true || typeof row.check_code !== 'string')) throw new Error('Preflight gate failed');
  if (new Set(rows.map((row) => row.check_code)).size !== 18) throw new Error('Preflight gates must be unique');
...
function validatePostcheck(rows) {
  if (!Array.isArray(rows) || rows.length !== 11 || rows.some((row) => row.passed !== true)) throw new Error('Postcheck did not pass 11/11');
}
async function preflight({ repo, stateFile, client, runTests = () => cp.execFileSync('npm', ['test'], { cwd: repo, stdio: 'inherit' }) }) {
  verifyGit(repo); runTests();
  ...
  writeState(stateFile, state, currentBinding, repo); readState(stateFile, currentBinding, repo);
...
async function execute({ repo, stateFile, client, confirmation }) {
  if (confirmation !== 'E2-T5-V2-LIVE') throw new Error('Exact human confirmation is required');
  verifyGit(repo); const currentBinding = binding(repo); let state = readState(stateFile, currentBinding, repo);
  if (state.consumed || state.transactionSent || state.phase !== 'approval-ready') throw new Error('State cannot send a transaction');
  state = { ...state, phase:'transaction-sent', transactionSent:true }; writeState(stateFile, state, currentBinding, repo);
  let evidence; let transactionError;
  ...
  return Object.freeze({ operation:OPERATION, status:'PASS', evidenceVersion:evidence.evidence_version, transactionRequests:1, transactionRetries:0, postcheck:'11/11 PASS', postcheckRequests:1, postcheckRetries:0, state:'CONSUMED' });
}
module.exports = Object.freeze({ APPROVED_MAIN_SHA, OPERATION, VERSION, SQL, binding, checksum, execute, postcheckSql, preflight, validatePostcheck, validatePreflight, verifyGit });
