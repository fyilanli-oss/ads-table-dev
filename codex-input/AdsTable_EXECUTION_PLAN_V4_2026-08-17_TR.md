...
- **R6-D5-A1 Klaviyo kampanya tarih teşhisi — Repository implementation pending:** R6-D5-A canlı sonucundaki “kapalı gönderim tarihi bulunamadı” durumunu tahminsiz ayırmak için connected Klaviyo hesabında email/SMS kampanyaları salt okunur sayılır. Dış sonuç yalnız toplam kampanya, Sent kampanya, scheduled_at bulunan Sent kampanya ve scheduled_at bulunmayan Sent kampanya adetlerini taşır; campaign/account/token kimliği veya adı taşımaz. OAuth, token lifecycle, hesap/metric seçimi, Campaign/Flow mapper, FX, Dataset writer ve schedule/backfill değiştirilmez. Dataset V2 yazımı ve production activation kesinlikle kapalı kalır; canlı provider çağrısı deployment sonrası ayrı merchant kabul adımıdır.
