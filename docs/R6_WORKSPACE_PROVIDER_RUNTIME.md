...
### R6-D3-F Meta bağımsız Disconnect — contract PASS / implementation gate
...
Her provider'ın Connect → Connected → Disconnect → temiz Reconnect zinciri kendi R6-D kabulünde tamamlanacaktır. Meta Disconnect yalnız Meta provider yetkisini ve canonical credential/account seçimini temizler; Klaviyo, reporting currency, legacy history ve Dataset V1/V2 korunur. Provider revoke yerel cleanup'tan önce gelir ve hata fail-closed bırakır. Bu karar paketinde provider teması veya Supabase mutation yapılmamıştır. Sözleşme `contracts/r6d3f-meta-independent-disconnect-v1.json`, analist kaydı `docs/R6D3F_META_INDEPENDENT_DISCONNECT.md` içindedir. Sıradaki kapı Meta Disconnect uygulamasıdır; Google Ads henüz başlamaz.
