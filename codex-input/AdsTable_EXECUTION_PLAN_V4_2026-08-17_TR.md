...
**Durum:** `Verification` — PR #117 merge edildi ve `main` kontrolleri başarılıdır. PR #118–#120 sonrası test reklam hesapları seçildi ve modal döngüsü kapandı; bu noktada OAuth token + account selection tamamlandığı için `Connected` görünmesi doğrudur, Refresh hatası bağlantıyı geçersiz kılmaz. İlk gerçek Refresh ve PR #121 sonrası tekrar, aynı PostgreSQL `invalid input syntax for type uuid: "[object Object]"` hatasıyla fail-closed oldu. PR #121 yalnız `source_job_id` sınırını normalize etti; tekrar eden hata Dataset V2'nin diğer UUID girdisi olan user identity/auth context sınırının açık kaldığını kanıtladı. Corrective revizyon hem `user_id` hem `source_job_id` değerlerini Google live runtime girişinde scalar UUID'ye normalize edip currency lookup ve persistence boyunca aynı değerleri kullanır. Canlı tekrar kontrolü yapılmadan E5-T7 `Done` sayılmaz; Meta ile aynı completed/no-error kabul sonucu doğrulanmadan E6 başlamaz.
