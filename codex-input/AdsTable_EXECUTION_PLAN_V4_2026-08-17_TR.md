...
| R5 | Mevcut Klaviyo account konsolidasyonu | E7 + E10-T6 | `In progress / R5-A Done; R5-B reauthorization required; R5-C preparation` — server-only binding kayıtlıdır. Tek no-refresh canlı doğrulama `409` ile fail-closed durdu; eski grant kanoniğe taşınmaz. Kontrollü temiz reset ayrı işlem-anı onayıyla eski refresh token'ı bir kez revoke edip embedded satırı ancak provider başarısından sonra `revoked` yapacaktır. |
...
- **R5-C preparation merge + execution gate:** PR #232 merge commit `cb51f91f257622c9e6715fd2ae960cf9a0b596d4` production'a alındı; Vercel target `production`, `dev.adstable.app` alias'ı ve `READY` durumu doğrulandı. Canlı salt-okunur preflight tek embedded connected Klaviyo grant'i, aktif hesap, refresh token, aktif human binding, boş canonical Klaviyo ve yerinde legacy write-freeze kapılarını birlikte `PASS` doğruladı. Execution gate resmi Shopify `s-modal` ile hazırlanır: modal açma/Cancel etkisiz, yalnız **Remove connection** exact confirmation ile session-bound POST route'unu çağırır. Revoke başarılı olmadan yerel status değişmez; başarı sonrası Connect yeniden açılmaz ve yeni OAuth R6/R7'ye kadar kapalı kalır. Execution gate henüz merge/deploy edilmemiştir; canlı revoke ve Supabase mutation yapılmamıştır.
