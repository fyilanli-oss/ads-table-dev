...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-D2 Klaviyo live PASS; R6-D3 Meta full lifecycle live PASS; R6-D4-G Google Ads lifecycle PASS; R6-D5-A Klaviyo historical read-only pending` — Google Sheets ve GA4 parked kalır. Google Ads bağlantı, üç hesap, read-only ve kontrollü verified-empty veri kapıları canlıda geçmiştir. Son yaşam döngüsü kapısı bağımsız local Disconnect, Cancel ve temiz Reconnect kabulüdür. |
