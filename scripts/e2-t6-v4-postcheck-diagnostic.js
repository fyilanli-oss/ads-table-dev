...
async function main() {
  ...
  const stateFile = process.env.ADS_TABLE_OPERATOR_STATE || path.join(os.homedir(), '.local', 'state', 'ads-table', 'e2-t6-v4.json');
...
if (require.main === module) main().catch((error) => { process.stderr.write(`${JSON.stringify({ operation: 'e2_t6_rls_v4', status: 'DIAGNOSTIC_FAIL_CLOSED', safeCode: error && error.safeCode || 'DIAGNOSTIC_PREREQUISITE_FAILED' })}\n`); process.exitCode = 1; });
