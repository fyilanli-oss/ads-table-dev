...
PR #265 production'a dağıtıldıktan sonra açık onaylı tek kontrollü kabul `attempted 0`, `persisted 0`, `verified empty true` sonucu verdi. Supabase postcheck Dataset V2 toplam/Meta, aktif Meta schedule, açık Meta job ve browser grant sayılarını `0` olarak doğruladı; canonical `connected · 1 account` ve legacy Meta `1` korundu. Sahte satır üretilmedi. Non-empty fiziksel UPSERT henüz gözlenmemiştir. Sözleşme `contracts/r6d3e-meta-controlled-dataset-acceptance-v1.json`, analist kaydı `docs/R6D3E_META_CONTROLLED_DATASET_ACCEPTANCE.md`, redacted kanıt `docs/security/evidence/R6D3E_META_CONTROLLED_DATASET_LIVE_ACCEPTANCE_2026-09-25.json` içindedir. R6-D3-E veri kabulü kapanmıştır; tam Meta yaşam döngüsü R6-D3-F bağımsız Disconnect ve temiz Reconnect kabulüne kadar açıktır.

### R6-D3-F Meta bağımsız Disconnect — repository implementation PASS / production acceptance pending

Eski `server.js + dashboard.html` referansında bulunan Meta Disconnect yaşam döngüsünün yeni embedded yüzeye taşınmadığı doğrulanmıştır: embedded Meta kartı yalnız Connected rozeti gösterir, session-bound Disconnect route'u ve canonical Meta cleanup operasyonu yoktur. Mevcut `workspace_provider_connections` şeması `disconnected`, `disconnected_at` ve `connection_version` alanlarıyla hedef durumu destekler; yeni migration gerekmez.

Her provider'ın Connect → Connected → Disconnect → temiz Reconnect zinciri kendi R6-D kabulünde tamamlanacaktır. Meta için Shopify-native warning modalı, session-bound endpoint, revoke-first service ve optimistic canonical cleanup uygulandı; 55 odaklı repository testi PASS verdi. Meta Disconnect yalnız Meta provider yetkisini ve canonical credential/account seçimini temizler; Klaviyo, reporting currency, legacy history ve Dataset V1/V2 korunur. Revoke hatası local cleanup yapmadan fail-closed bırakır. Production deployment, canlı Disconnect ve temiz Reconnect henüz doğrulanmamıştır; Supabase migration/mutation veya provider teması yapılmamıştır. Sözleşme `contracts/r6d3f-meta-independent-disconnect-v1.json`, analist kaydı `docs/R6D3F_META_INDEPENDENT_DISCONNECT.md` içindedir. Sıradaki kapı production deployment ve merchant acceptance'tır; Google Ads henüz başlamaz.
