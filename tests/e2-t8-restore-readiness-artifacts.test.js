...
test('source inventory attempts are redacted, no-change, and stopped for business decision',()=>{assert.deepEqual(attempts.attempts.map(x=>x.safe_code),['SOURCE_INVENTORY_QUERY_FAILED','SOURCE_INVENTORY_MANAGEMENT_TRANSPORT_FAILED','SOURCE_INVENTORY_CONTRACT_FAILED','SOURCE_INVENTORY_OWNERSHIP_UNCLASSIFIED']);assert.deepEqual(attempts.attempts.map(x=>x.request_count),[1,1,1,1]);assert.ok(attempts.attempts.every(x=>x.state_capsule_created===false));assert.equal(attempts.schema_capture_requests,0);assert.equal(attempts.automatic_retries,0);assert.equal(attempts.production_mutations,0);assert.equal(attempts.next_gate,'STOP_PENDING_E2_T8_DEFER_DECISION');assert.doesNotMatch(JSON.stringify(attempts),/https?:|project.ref|authorization|bearer|password|token/i);});
