...
**Durum:** `Verification` — PR #117 merge edildi ve `main` kontrolleri başarılıdır. PR #118–#120 sonrası test reklam hesapları seçildi ve modal döngüsü kapandı; OAuth token + account selection tamamlandığı için `Connected` görünmesi doğrudur. PR #121 ve #122'nin UUID varsayımları production'da aynı PostgreSQL hatasını çözmedi; mevcut hata mesajı tablo/kolon/aşama belirtmediği halde kök neden kesinleştirilmeden düzeltme yapılması sapma olarak kaydedildi. Yeni corrective paket veri veya kimlik taşımayan aşama kodlarını job metadata'sına yazar: customer metadata, target currency, provider adapter, FX lookup, Dataset V2 write ve Standard/PMax ingest. Bu evidence görülmeden yeni alan düzeltmesi yapılmaz. Canlı teşhis ve ardından completed/no-error kabul doğrulanmadan E5-T7 `Done` sayılmaz; E6 başlamaz.
