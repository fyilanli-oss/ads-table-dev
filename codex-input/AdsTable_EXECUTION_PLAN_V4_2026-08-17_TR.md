...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-A+B+C+D1 Done; R6-D2-C1+C2+C3+C4+C5 live PASS; C6-A first write FAILED; C6-B diagnostics PASS; C6-C second attempt FAILED_PROVIDER_ACCOUNT / rows 0; C6-D token lifecycle repository PASS, deployment pending` — İkinci kontrollü deneme Account API aşamasında fail-closed durdu; Dataset V2 upsert oluşmadı ve satır sayısı `0` kaldı. Kök neden adayı, connected canonical runtime'ın kısa ömürlü access token için expiry/refresh yaşam döngüsünü uygulamamasıydı. C6-D bunu merchant'ı yeniden OAuth'a zorlamadan, encrypted refresh-token rotation ve tek 401 retry ile düzeltir. Tamamlanmış E4/E5/E7 ve satış metriği politikası bu pakette değiştirilmez. |
...
- **R6-D2-C6-D connected Klaviyo token lifecycle corrective — Repository PASS / deployment pending:** İnceleme canonical bağlantının encrypted refresh token ve `accounts:read` scope taşıdığını, fakat `access_token_expires_at` değerinin boş olduğunu ve connected runtime'ın token refresh uygulamadığını doğruladı. Yeni OAuth cevabındaki `expires_in` canonical kayda aktarılır. Connected runtime expiry bilinmiyorsa veya yaklaşmışsa access token'ı provider `/oauth/token` ile yeniler; provider'ın döndürdüğü en güncel refresh token aynı workspace connection'a encrypted envelope olarak optimistic connection version ile yazılır. Beklenmeyen provider `401` yalnız bir refresh + bir retry üretir; ikinci auth hatası veya `invalid_grant` merchant reauthorization ister. `403`, provider `429`, refresh `429`, refresh configuration ve 5xx durumları güvenli ayrı kodlardır. Aynı process'teki paralel istekler tek refresh paylaşır. Repository kapsam testleri `120/120` PASS; production deployment, gerçek token refresh/provider teması ve C6 yeniden denemesi yapılmadı. Metric seçimi, mapping ve Dataset writer değiştirilmez; tamamlanmış E4/E5/E7 yeniden geliştirilmez.
