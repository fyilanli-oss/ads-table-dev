...
- **R6-D4-B Google Ads token ve manager-context uygulaması — Repository PASS / R6-D4-C merchant acceptance gate:** Embedded Google token exchange artık refresh token, pozitif access expiry ve exact `adwords` scope olmadan canonical save'e geçmez. Pending veya connected canonical connection için server-side expiry refresh, refresh-token rotation, optimistic version koruması ve beklenmeyen auth hatasında yalnız bir refresh + bir retry uygulanır; `invalid_grant` veya ikinci auth hatası `GOOGLE_REAUTHORIZE` üretir ve legacy token'a düşmez. `ListAccessibleCustomers + customer_client` keşfinde manager hesaplar seçilemez kalırken her seçilmiş reklam hesabının doğrulanmış `login_customer_id` değeri `selected_accounts` JSONB envelope'unda korunur; refresh selection sırasında gerçekleşirse Save yeni connection version'ı kullanır. Mevcut JSONB alanı yeterli olduğu için migration yapılmadı; RLS/browser grant değişmedi. E5 yeniden geliştirilmedi; Google Sheets/GA4 parked kaldı. Bu paket production deployment, provider teması, Supabase mutation, Dataset V2 write, schedule/backfill veya Disconnect/revoke yapmadı. Contract `contracts/r6d4b-google-ads-token-context-v1.json`, analist kaydı `docs/R6D4B_GOOGLE_ADS_TOKEN_CONTEXT.md` içindedir.
- **R6-D4-C1 Google Ads hesap keşfi corrective — Repository PASS / production diagnostic gate:** Merchant OAuth callback ve canonical pending status production loglarında başarılı, `/api/shopify/providers/google_ads/accounts` ise `503` olarak kanıtlandı. Eski `server.js + dashboard.html` test hard-route'u ile embedded gerçek `ListAccessibleCustomers + customer_client` seçimi karşılaştırıldı. Embedded discovery; provider HTTP adımları yanında token lifecycle, JSON response, customer transform ve verified-account shape aşamalarını güvenli biçimde ayırır. Yalnız HTTP `401` reauthorization sayılır; `403` developer-token/Google Ads authorization kanıtını korur ve boşuna refresh çalıştırmaz. Yalnız upstream HTTP status, Google hata sınıfı ve request ID server loguna taşınır; token/developer-token/customer ID/provider mesajı/body loglanmaz. Canonical `GOOGLE_ADS_DEVELOPER_TOKEN` tercih edilir; eski `GOOGLE_DEVELOPER_TOKEN` yalnız server-side fallback'tir. Pending connection korunur; yeniden OAuth, Supabase mutation, Dataset V2 write, schedule/backfill veya Disconnect yapılmadı. Analist kaydı `docs/R6D4C1_GOOGLE_ACCOUNT_DISCOVERY_CORRECTIVE.md` içindedir.
- **R6-D4-C Google Ads bağlantı ve hesap seçimi — Production merchant acceptance PASS / 3 verified accounts / R6-D4-D next:** PR #276 merge commit `4b34eb4bfe33ca3ac67091b57e5ccd38c6ac25ce` production'da `READY` olmuş ve `dev.adstable.app` alias'ı doğrulanmıştır. Merchant'ın korunmuş pending grant'i yeniden OAuth istemeden hesap seçim modalını açmış; kullanıcı test erişimi altında görünen üç hesabı seçip kaydetmiş ve reload sonrasında `Connected · 3 accounts` durumunu doğrulamıştır. Production runtime kanıtında accounts GET ve selection POST `200` dönmüştür. Supabase salt-okunur postcheck tek canonical Google Ads satırını `connected`, üç seçilmiş hesabı doğrulanmış şekil, encrypted credential envelope'larını mevcut, hesap doğrulama/connected zamanlarını dolu ve yetkilendirme adapter'ını `shopify_verified_session` olarak doğrulamıştır. Dataset V2 Google Ads satırı `0` kalmıştır. Hesap, workspace veya token değeri kanıta alınmamıştır. Bu PASS yalnız bağlantı ve hesap seçimidir; E5 yeniden geliştirilmemiş, veri preflight/write, schedule/backfill veya Disconnect yapılmamıştır. Redacted kanıt `docs/security/evidence/R6D4C_GOOGLE_ADS_CONNECTION_LIVE_ACCEPTANCE_2026-09-26.json` içindedir. Sıradaki kapı kodlama değil, R6-D4-D salt-okunur Google Ads preflight analist brief'idir.
- **R6-D4-D Google Ads salt-okunur runtime preflight — Production read-only acceptance PASS / verified empty / Dataset V2 gate:** Üç seçilmiş canonical hesapta tamamlanmış E5 müşteri metadata, Standard Ads, Performance Max, conversion mapping ve Time/FX motoru production'da salt okunur çalıştırılmıştır. Merchant sonucu `PASS — 3 account(s), 0 verified row(s) across Standard and Performance Max. Time and FX checks succeeded. Dataset V2 writes: 0.` olmuştur. Supabase salt-okunur son kontrolü tek canonical Google bağlantısını `connected`, seçili hesap sayısını `3`, encrypted access/refresh token zarflarını ve reporting currency kaydını mevcut; workspace/Google Dataset V2 satırlarını `0`, sentetik Google satırını `0` ve tarayıcı rol grant'ini `0` doğrulamıştır. Dataset V2/V1 write, schedule, backfill, yeni metrik, Google Sheets/GA4 aktivasyonu, Disconnect veya production veri aktivasyonu yapılmamıştır. Redacted kanıt `docs/security/evidence/R6D4D_GOOGLE_READ_ONLY_LIVE_ACCEPTANCE_2026-09-26.json` içindedir. R6-D4-D tamamlanmıştır; sıradaki kapı açık production onayı gerektiren R6-D4-E kontrollü Dataset V2 kabulüdür.
- **R6-D4-E Google Ads kontrollü Dataset V2 kabulü — Production verified-empty PASS / data gate closed / Disconnect gate next:** PR #280 merge commit `6b4d4c1abb7a9a3fe83959c176a0213abd3ffa90` Security/Full Regression ve Vercel kontrolleri geçerek production'a dağıtılmıştır. Exact action-time confirmation ile üç canonical hesap için tamamlanmış E5 Standard, Performance Max, conversion mapping, Time/FX ve R6-D4-D workspace runner tek kez çalıştırılmıştır. Merchant sonucu `PASS — attempted: 0, persisted: 0, verified empty: true.` olmuştur. Supabase salt-okunur son kontrolü tek canonical Google bağlantısını `connected`, üç seçili hesabı geçerli `login_customer_id` bağlamıyla, access/refresh token zarflarını, access expiry ve merchant reporting currency kaydını mevcut doğrulamıştır; Dataset V2 toplam/workspace/Google/sentetik Google satırları ile browser grant sayısı `0` kalmıştır. Sahte satır üretilmemiş ve non-empty fiziksel UPSERT gözlenmemiştir. Schedule, backfill, Dataset V1, Google Sheets, GA4, yeni metrik ve production aktivasyonu açılmamıştır. Contract `contracts/r6d4e-google-controlled-dataset-acceptance-v1.json`, analist kaydı `docs/R6D4E_GOOGLE_CONTROLLED_DATASET_ACCEPTANCE.md`, redacted kanıt `docs/security/evidence/R6D4E_GOOGLE_CONTROLLED_DATASET_LIVE_ACCEPTANCE_2026-09-26.json` içindedir. R6-D4-E veri kapısı tamamlanmıştır; sıradaki ayrı paket R6-D4-F Google bağımsız Disconnect analist brief'idir.
- **R6-D4-F Google Ads bağımsız Disconnect — Repository PASS / production acceptance pending:** Shopify-native warning modalı, non-destructive Cancel, session-bound exact-confirmation endpoint’i ve optimistic canonical cleanup hazırlanmıştır. Google Ads, parked Google Sheets ve parked GA4 geçmişte aynı OAuth client’ını kullandığı için provider global revoke kesinlikle yapılmaz; yalnız bu workspace’in canonical `google_ads` credential, expiry, scope ve seçilmiş hesap alanları temizlenir. Reporting currency, Meta, Klaviyo, tarihsel Dataset V2/legacy veri ve parked Google servisleri korunur. Migration gerekmez. Repository kapsamı ve ilgili regresyonlar PASS’tir. Merchant `Connected · 3 accounts → Cancel → Connected · 3 accounts → Disconnect → Not connected → temiz OAuth/1–3 hesap → Connected` zincirini ve iki Supabase postcheck’i tamamlamadan production PASS yazılamaz. Contract `contracts/r6d4f-google-independent-disconnect-v1.json`, analist kaydı `docs/R6D4F_GOOGLE_INDEPENDENT_DISCONNECT.md` içindedir.
- **R6-D4-G Google Sheets + GA4 kontrollü emeklilik ve gerçek Google Ads revoke — Production runtime/data acceptance PASS / Cloud admin hygiene pending:** PR #283 merge commit `0998f41124b2dade4238c9a2637f347da8568955` Security/Full Regression PASS sonrasında production’a dağıtıldı. İşlem-anı onayıyla Google grant revoke edildi; Shopify Google Ads `Not connected`, canonical credential/account alanları temiz ve connection version `12` oldu. Legacy Google Sheets encrypted credential satırı kaldırıldı ve connected durumu kapatıldı; spreadsheet metadata korunmuştur. GA4 tarihsel `340` job ve `2` pasif schedule korunmuş, aktif job/schedule `0/0`, Dataset V2 `0`, reporting currency `TRY`, Meta/Klaviyo bağlı kalmıştır. Temiz Google OAuth yalnız `adwords` scope ile ilk-izin uyarısını yeniden göstermiş; 3 doğrulanmış hesapla canonical Google Ads `connected`, access/refresh/expiry mevcut ve version `13` olarak postcheck PASS vermiştir. Google Cloud Console’da kullanılmayan Sheets/Analytics API’lerinin kapatılması non-blocking idari temizliktir; ürün runtime’ı veya R6-D5 kapısını açıp kapatmaz. Redacted kanıt `docs/security/evidence/R6D4G_GOOGLE_LEGACY_RETIREMENT_LIVE_2026-09-26.json` içindedir.
- **R6-D5-A Klaviyo tarihsel test-verisi envanteri — Repository preparation / live read-only acceptance pending:** R6-D2 yeniden açılmaz; bağlantı, token lifecycle, metric binding, Campaign/Flow mapping ve Dataset writer değiştirilmez. Önceki C5/C6 kabulü yalnız güvenli `now-48h` gününü denediği için, provider `campaigns:read` kapsamıyla yalnız `Sent` email/SMS kampanyalarının `scheduled_at` değerleri account timezone içinde kapalı business date adaylarına çevrilir. En yakın kapalı gönderim günü mevcut workspace runner üzerinden salt okunur değerlendirilir; kullanıcıdan workspace/date/account alınmaz, dış sonuç campaign/account/token ID taşımaz. Bu aşama Dataset V2 yazmaz, schedule/backfill/production activation açmaz ve sentetik satır üretmez. Canlı sonuç non-empty ise ilk kontrollü tarihsel Dataset V2 yazımı R6-D5-B olarak ayrı açık işlem-anı onayı ister; sonuç empty ise ikinci tarih adayı ancak ayrı teşhis kararıyla incelenir.
- **R6-D5-A1 Klaviyo kampanya tarih teşhisi — Production read-only acceptance PASS / no Sent campaign / Dataset V2 gate closed:** PR #286 merge commit `309db7a756cde18f44ba3b0f51d7a2611ebae7f0` Security/Full Regression PASS sonrasında production'a dağıtılmış ve `dev.adstable.app` alias'ı doğrulanmıştır. Merchant salt-okunur sonucu `PASS — 2 campaign(s), 0 Sent; 0 with scheduled_at, 0 without scheduled_at. No closed sent date. Dataset V2 writes: 0.` olmuştur. Supabase postcheck Dataset V2 toplam/Klaviyo/sentetik satırlarını `0/0/0` doğrulamıştır. Sonuç kampanya yokluğunu değil, iki kampanyanın hiçbirinin Sent durumda olmadığını kanıtlar; Campaign/Flow satırı üretilmemiştir. OAuth, token lifecycle, hesap/metric seçimi, mapper, FX, Dataset writer, schedule/backfill ve production activation değişmemiştir.
- **R6-D5-A2 Klaviyo Flow/Event salt-okunur envanteri — Repository implementation pending:** Sent kampanya bulunmayan test hesabında bağımsız Flow durumunu ve sentetik Event varlığını kanıtlamak için connected Klaviyo hesabından yalnız aggregate teşhis okunur. Sonuç Flow toplamı ile Draft/Manual/Live/other adetlerini; taranan event adedini, event tarih aralığını, attribution adedini ve yalnız allowlist edilmiş Klaviyo/e-ticaret metric kategori adetlerini taşır. Profil/email, Flow/campaign/event/metric ID veya adları dışarı verilmez. Event verisi Campaign/Flow performans satırı sayılmaz ve Dataset V2'ye yazılmaz. OAuth, token lifecycle, hesap/metric seçimi, mevcut Campaign/Flow mapper, FX, Dataset writer ve schedule/backfill değişmez; canlı provider çağrısı deployment sonrası ayrı merchant kabul adımıdır.
...
- **E9-T1 — Done; provider scope revised:** İlk hazırlama yesterday/finalized ardından today/provisional; Meta ve Google Ads için aynı OAuth grant'i altında provider tarafından doğrulanmış 1–3 reklam hesabı, Klaviyo için tek doğrulanmış hesap; TikTok, Pinterest ve diğer parked provider'lar dışarıda; eski tarihçe otomatik değil; 14. günde günlük birikim devam eder.
