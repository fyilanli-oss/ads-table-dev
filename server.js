...
async function getSubscriptionForLifecycle(userId){await expireTrialsIfNeeded();const{data,error}=await supabaseAdmin.from("subscriptions").select("status,trial_end_date,deleted_at,hard_delete_at").eq("user_id",userId).maybeSingle();if(error)throw error;return data}
