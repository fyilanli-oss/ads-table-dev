...
const OBJECT_KEYS = ['object_key','migration_version','effect','overlap_group','pending_capture_checksum'];
...
function validatePreparation(scope, classification, manifest) {
  ...
  for (const migration of classification.migrations) {
    ...
    for (const object of migration.objects) { if (!exactKeys(object, OBJECT_KEYS) || object.migration_version !== migration.version || object.pending_capture_checksum !== true || !/^(?:relation|function|constraint|policy|grant_state):/.test(object.object_key)) errors.push('OBJECT_CLASSIFICATION'); }
  }
...
module.exports = { MAIN_SHA, SCOPE_KEYS, MANIFEST_KEYS, MIGRATION_KEYS, OBJECT_KEYS, SENSITIVE, sha256, validatePreparation, validateTarget };
