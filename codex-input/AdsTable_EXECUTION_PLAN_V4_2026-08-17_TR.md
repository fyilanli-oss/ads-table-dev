...
**Evidence:** `supabase/migrations/20260819120000_harden_platform_connection_tokens_service_role_grants.sql`, `tests/provider-token-schema.test.js`, `node --test tests/provider-token-schema.test.js`, `npm test`, `node --check tests/provider-token-schema.test.js` ve `git diff --check`. Canlı migration çalıştırılmadı; evidence secret, key, token, PII veya kullanıcı kimliği içermez.
