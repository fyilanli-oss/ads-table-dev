...
- **E10-T6 — In progress; A Done, B in progress — Capability/readiness, development bootstrap ve gerekçeli sync:** E10-T6-A resmi offline readiness PASS; E10-T6-B için açık development onayı alındı, app/store kullanıcı tarafından oluşturuldu ve Shopify-managed installation bootstrap düzeltmesi başladı. Shopify credential'ları çalışan process'te 4/4 görünür; Development Store smoke encryption keyring'in aynı process'e injection'ını bekliyor. Webhook/initial sync ancak C5-A ve resmi capability sonucu gerçekten gerektirirse açılır.
...
**Durum:** `In progress / Blocked — external keyring daha önce doğrulandı; bu Codex smoke process'inde key map görünmüyor`.
...
- **Managed-installation düzeltmesi:** Güncel managed modelde uygulama install OAuth callback'i üretmez; Shopify install/scope onayını yönetir. Embedded App Home session token'ı `POST /api/shopify/bootstrap` üzerinden doğrulanır, offline token exchange ve Admin API shop identity kontrolünden sonra binding + encrypted token persistence atomic tamamlanır. Server-only Supabase schema/RPC/service ve composition-root kaydı repository'de hazırdır; açık onayla remote Supabase migration ve redacted postcheck `PASS_MIGRATION_ONLY` tamamlandı. Shopify env görünürlüğü 4/4 PASS; Development Store smoke encryption keyring process injection'ını bekliyor. Redacted contract ve kalan smoke adımları `docs/E10_T6B_MANAGED_INSTALLATION_BOOTSTRAP.md` içindedir.
- **2026-09-10 smoke sonucu:** Dört Shopify girdisi değerleri ve uzunlukları gösterilmeden 4/4 görünür ve Development Store domain'i canonical doğrulanmıştır; remote Supabase salt-okunur güvenlik/migration postcheck yeniden PASS'tir. Tekrarlanabilir redacted preflight somut olarak çalıştırıldı. Keyring girdileri yalnız bu Codex process'inde 1/2 görünürdür: active key kimliği `process.env` içindedir, encryption key map değildir. Bu sonuç external secret'ın yokluğu değildir; E1 kabulü production provisioning, encrypted backfill ve encrypted-only runtime'ın tamamlandığını kanıtlar. Mevcut keyring değiştirilmeden Codex task secret scope/injection yolu incelenmelidir. Vercel deployment/CI PASS'tir; operator App URL route probe ağ geçidinde engellenmiştir ve bu sonuç uygulama arızası kanıtı değildir. Credentialsız Development Store reachability probe'u HTTP 4xx yanıtıyla ağ temasını doğrulamış, authentication/install kabulü üretmemiştir. Repository'deki eksik browser başlatıcısı tamamlanmıştır: root App URL embedded yükü server-rendered App Home'a aktarır; App Bridge ID token'larıyla iki idempotent bootstrap ve ardından session doğrulaması çalıştırılır, hassas materyal browser storage/DOM/log'a konmaz. Gerçek session/ID token, token exchange, Admin Shop identity, binding, encrypted persistence ve reopen/reinstall henüz çalıştırılmamıştır; production teması olmamıştır. E10-T6-B `In progress / Blocked`, E10-T6-C kapalıdır. Redacted evidence `artifacts/e10-shopify/e10-t6b-development-store-smoke.json` içindedir.
