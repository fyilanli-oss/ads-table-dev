...
function mapGoogleStandardAd(input,{userId,workspaceId,customer,sourceJobId=null}={}){
  ...
  const workspaceMode=workspaceId!==undefined&&workspaceId!==null;
  const identity={user_id:workspaceMode?null:text(userId,'userId'),...(workspaceMode?{workspace_id:text(workspaceId,'workspaceId')}:{}),platform:'google',traffic_type:'paid',source_system:'google_ads',channel:null,platform_account_id:text(customer.id,'customer.id'),date};
