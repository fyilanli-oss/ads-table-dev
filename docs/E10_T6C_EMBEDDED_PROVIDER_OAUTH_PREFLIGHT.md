...
Koordinatörün Vercel credential görünürlüğü, Vercel'e temas etmeyen ve secret değeri/uzunluğu yazmayan `scripts/e10-t6c-vercel-auth-readiness.sh` ile kontrol edilir. `VERCEL_AUTH_AVAILABLE` yalnız aktivasyon operatorünün zorunlu tuttuğu `VERCEL_TOKEN` bu process'te mevcutsa döner; CLI login veya `.vercel` bağlantısı token yerine kabul edilmez. `VERCEL_AUTH_UNAVAILABLE` durumunda aktivasyon denenmez ve credential kullanıcı mesajına, issue'ya ya da log'a taşınmaz.
