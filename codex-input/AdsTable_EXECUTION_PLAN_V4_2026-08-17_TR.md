...
- **E1-T6 — `In progress` — Token protection:** E1-T6A vault hazır, E1-T6B canlı schema/grant acceptance tamamlanmış, E1-T6C feature-flagged encrypted write/legacy-read artefaktı ve E1-T6D dry-run-only operator harness hazırdır. Production key provisioning yapılmadı ve flag kapalıdır; canlı dry-run, ayrı write-mode kararı, plaintext doğrulama ve legacy retirement sonraki kapılardır.
...
**Gerçekleşen (E1-T6D — `artefact ready / production dry-run pending`):** Mevcut resumable runner için yalnız dry-run sunan, fail-closed argüman/config doğrulamalı ve allowlist/redacted tek JSON evidence üreten bağımsız operator harness ile ince CLI hazırlandı; otomatik testleri geçti. Harness runner'ı koşulsuz `dryRun:true` ile çağırır ve write-mode girdisi sunmaz. Canlı dry-run yapılmadı, production key/secret provisioning yapılmadı, encryption flag açılmadı, write-mode backfill çalıştırılmadı ve plaintext tokenlar değiştirilmedi. Bu nedenle E1-T6D `Done` değildir; production dry-run ve sonraki operasyon kapıları beklemektedir.
...
3. **E1-T6D — Backfill/rotation (`artefact ready / production dry-run pending`):** Resumable motor ve dry-run-only redacted operator harness hazır; production key provisioning, canlı dry-run, write-mode backfill ve flag activation yapılmadı.
...
**Evidence:** E1-T6B kapsamında ilk ve corrective migration canlıda uygulandı; post-migration grant/RLS acceptance tamamlandı. PR #11 kapsamında canlı dry-run yapılmadı, production key provisioning yapılmadı, encryption flag açılmadı ve write-mode backfill çalıştırılmadı. Repository evidence'ı `supabase/migrations/20260819120000_harden_platform_connection_tokens_service_role_grants.sql`, `tests/provider-token-schema.test.js`, `tests/provider-token-backfill-operator.test.js`, ilgili hedefli test komutları, `npm test`, `node --check` ve `git diff --check` ile sınırlıdır; secret, key, token, PII veya kullanıcı kimliği içermez. E1-T6D `Done` değildir ve `artefact ready / production dry-run pending` durumundadır.
