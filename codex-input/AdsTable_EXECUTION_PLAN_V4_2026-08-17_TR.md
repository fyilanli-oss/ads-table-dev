...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-D2 Klaviyo live PASS; R6-D3-C Meta connection/account-selection live PASS; R6-D3-D read-only preflight repository PASS, production acceptance pending` — Meta legacy kaydı korunmuş ve expired token'ı kullanılmamıştır. Temiz embedded OAuth ve bir provider-doğrulanmış hesap seçimi production'da tamamlanmıştır. Tamamlanmış E4 motorunu workspace authority'ye bağlayan salt-okunur preflight hazırdır; yeni metrik veya mapper üretilmemiştir. Production Meta Insights/Time/FX sonucu ve Dataset V2 kabulü ayrıca kanıtlanmadan R6-D3 `Done` değildir. |
...
- **R6-D3-D Meta workspace salt-okunur preflight — Repository PASS / production read-only acceptance pending:** Tamamlanmış E4 Meta Client, Campaign→AdSet→Ad mapper, sabit metrik sözleşmesi ve Time/FX katmanı yeniden geliştirilmeden workspace authority'ye bağlandı. Canonical `connected` Meta kaydındaki 1–3 seçili hesabın tamamı `/me/adaccounts` ile yeniden doğrulanır; her hesap kendi provider currency ve timezone bilgisiyle önceki kapanmış business date için günlük Ad Insights okur. Reporting currency yalnız merchant-selected workspace setting'den gelir. Browser workspace/tarih/currency sağlayamaz; response token, hesap/entity kimliği veya ham metrik taşımaz. Normal Data Sources ekranı değişmez; operatör yüzeyi yalnız `?acceptance=r6d3-meta` ile görünür. Dataset V2/V1 write, schedule, backfill ve production activation yoktur. Sözleşme `contracts/r6d3d-meta-read-only-preflight-v1.json`, analist kaydı `docs/R6D3D_META_READ_ONLY_PREFLIGHT.md` içindedir. Gerçek Meta provider teması ve production PASS ayrı açık kabul kapısıdır.
