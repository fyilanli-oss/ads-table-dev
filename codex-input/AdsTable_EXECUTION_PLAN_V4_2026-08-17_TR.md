...
3. **E1-T6D — Backfill/rotation (`artefact ready / activation pending`):** Stable `user_id + platform` keyset cursor ile devam edebilen, batch-limitli, dry-run varsayılanlı ve idempotent backfill/rotation runner hazırdır. Sonuçlar yalnız sayım ile hashed user reference/platform/error-code evidence taşır; token, gerçek user ID veya exception mesajı taşımaz. Scheduled/queued refresh, disconnect revoke ve Google Sheets reconnect yollarındaki doğrudan plaintext connection okumaları merkezi encrypted hydration sınırına alınmıştır. Production key provisioning, dry-run ve kontrollü write acceptance tamamlanmadan çalıştırılmaz.
