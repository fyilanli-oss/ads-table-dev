...
**C2I-V9 real-device Shopify-native acceptance — FAIL:** 13 Eylül 2026 gerçek cihaz koşusunda uygulama Shopify Admin içinde açıldı, fakat App Home `SHOPIFY_INSTALL_PERSISTENCE_FAILED` ile durdu. Ekran görüntüsü ve referans kimliği repository'ye alınmadı. V9 `PASS` değildir; sıradaki çalışma yalnız install persistence düzeltmesidir. OAuth, consent, callback, account discovery, provider API veya Production mutation yetkisi verilmez. Ayrıntılı sözleşme `contracts/shopify/e10-t6-c2i-v9-real-device-acceptance.json` ve `docs/E10_T6_C2I_V9_REAL_DEVICE_ACCEPTANCE.md` içindedir.
