...
**C2I-V9 real-device Shopify-native acceptance — execution approved / human text confirmation pending:** Gerçek cihaz kabulü için ekran görüntüsü, PNG, SHA-256, metadata temizliği, evidence manifesti, validator veya GitHub yüklemesi zorunlu değildir. Kullanıcı Shopify Admin içinde App Home ve Data Sources/Platforms ekranlarını gözlemler, hiçbir Provider Connect düğmesine basmaz ve iki ekran açıldıysa tek satırlık insan onayı verir. Bu onay gelmeden V9 `PASS` değildir; OAuth, consent, callback, account discovery, token exchange, provider API, deployment veya Production mutation yapılmaz. Ayrıntılı sözleşme `contracts/shopify/e10-t6-c2i-v9-real-device-acceptance.json` ve `docs/E10_T6_C2I_V9_REAL_DEVICE_ACCEPTANCE.md` içindedir.
