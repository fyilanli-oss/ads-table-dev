...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-A+B Done; R6-C preparation` — canlı preflight `PASS_PREPARATION_ONLY`; ortak authority→connection→currency→runner→workspace V2 sınırı kodlandı fakat production'a kaydedilmedi. Aktivasyon migration'ı hazırlanır; gerçek provider kabulü R7-A'nın ilk currency ve canonical connection kaydından sonra R6-D'de yapılır. |
| R7 | Currency-first ve Shopify-native Connect/Disconnect UX | E10-T5 + E10-T6-C2I-V10 | `R7-A blocked by R6-B+C; R7-B blocked by R6-D` — R7-A Currency→Data Sources→explanation modal→OAuth→verified account→conditional Klaviyo cost foundation'ını açar; R6-D gerçek provider kabulünden sonra R7-B Connected ve warning modal kullanan Disconnect deneyimini tamamlar. |
