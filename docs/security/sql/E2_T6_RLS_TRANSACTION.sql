...
begin
  begin
    update public.performance_dataset_rows_v2 set entity_name=entity_name where entity_key='e2_t6_rls_v1:a'; get diagnostics v_count = row_count;
    raise exception using errcode='Z0001',message='local rollback sentinel';
  exception when sqlstate 'Z0001' then null;
    when insufficient_privilege then v_outcome := 'DENIED_BY_PRIVILEGE'; v_count := 0; v_state := sqlstate;
    when others then v_outcome := 'UNEXPECTED_SQLSTATE'; v_count := 0; v_state := sqlstate;
...
begin
  begin
    delete from public.performance_dataset_rows_v2 where entity_key='e2_t6_rls_v1:a'; get diagnostics v_count = row_count;
    raise exception using errcode='Z0001',message='local rollback sentinel';
  exception when sqlstate 'Z0001' then null;
    when insufficient_privilege then v_outcome := 'DENIED_BY_PRIVILEGE'; v_count := 0; v_state := sqlstate;
    when others then v_outcome := 'UNEXPECTED_SQLSTATE'; v_count := 0; v_state := sqlstate;
...
begin
  begin
    update public.performance_dataset_rows_v2 set entity_name=entity_name where entity_key='e2_t6_rls_v1:b'; get diagnostics v_count = row_count;
    raise exception using errcode='Z0001',message='local rollback sentinel';
  exception when sqlstate 'Z0001' then null;
    when insufficient_privilege then v_outcome := 'DENIED_BY_PRIVILEGE'; v_count := 0; v_state := sqlstate;
    when others then v_outcome := 'UNEXPECTED_SQLSTATE'; v_count := 0; v_state := sqlstate;
...
begin
  begin
    delete from public.performance_dataset_rows_v2 where entity_key='e2_t6_rls_v1:b'; get diagnostics v_count = row_count;
    raise exception using errcode='Z0001',message='local rollback sentinel';
  exception when sqlstate 'Z0001' then null;
    when insufficient_privilege then v_outcome := 'DENIED_BY_PRIVILEGE'; v_count := 0; v_state := sqlstate;
    when others then v_outcome := 'UNEXPECTED_SQLSTATE'; v_count := 0; v_state := sqlstate;
...
begin
  begin
    select count(*) into v_count from public.performance_dataset_rows_v2 where entity_key='e2_t6_rls_v1:a';
...
begin
  begin
    select count(*) into v_count from public.performance_dataset_rows_v2 where entity_key='e2_t6_rls_v1:b';
  exception when insufficient_privilege then v_outcome := 'DENIED_BY_PRIVILEGE'; v_count := 0; v_state := sqlstate;
    when others then v_outcome := 'UNEXPECTED_SQLSTATE'; v_count := 0; v_state := sqlstate;
  end;
