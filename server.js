...
function googleLandingPageViewQuery(level,range){const d=googleDateClause(range);if(level==="ad")return `SELECT segments.date, segments.ad_group, landing_page_view.resource_name, landing_page_view.unexpanded_final_url, metrics.clicks FROM landing_page_view WHERE ${d} AND metrics.clicks > 0 LIMIT 1000`;if(level==="adgroup")return `SELECT segments.date, segments.ad_group, landing_page_view.resource_name, landing_page_view.unexpanded_final_url, metrics.clicks FROM landing_page_view WHERE ${d} AND metrics.clicks > 0 LIMIT 1000`;return `SELECT segments.date, landing_page_view.resource_name, landing_page_view.unexpanded_final_url, metrics.clicks FROM landing_page_view WHERE ${d} AND metrics.clicks > 0 LIMIT 1000`}
