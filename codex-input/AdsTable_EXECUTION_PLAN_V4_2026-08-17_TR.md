...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-A+B+C+D1 Done; R6-D2-C1 deployment PASS; R6-D2-C2 product contract PASS; R6-D2-C3 next` — PR #250 ile session-bound Klaviyo preflight production'a ulaştı fakat ilk canlı deneme provider çağrısından önce `KLAVIYO_PREFLIGHT_NOT_CONFIGURED` ile durdu; R6-D2 PASS değildir. Global metric env bağı kaldırıldı; workspace + Klaviyo account kapsamlı Placed Order binding migration'ı ve PR #251 application deployment'ı PASS oldu. C2 satış kaynağı kullanıcı deneyimini teknik metric ID göstermeden, tek adayda ek ekran açmadan ve sıfır/çoklu adayda fail-closed kuralla dondurdu. Provider discovery, canonical metric yazısı ve Dataset V2 yazısı yapılmadı; tamamlanmış E4/E5/E7 adapter, mapping ve Dataset V2 yapıları yeniden geliştirilmez. |
