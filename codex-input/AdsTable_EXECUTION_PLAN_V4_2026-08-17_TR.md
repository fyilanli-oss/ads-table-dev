...
**Durum:** `Verification` — PR #117 merge edildi ve `main` kontrolleri başarılıdır. PR #118–#120 corrective zinciri sonrası test reklam hesapları seçim ekranında göründü ve modal döngüsü kapandı. Dashboard'un Google'a özel status endpoint'i ortak selection-aware durumu kullanmadığı için incomplete bağlantı yine Connected görünüyordu. İlk gerçek Refresh job'ı provider aşamasını geçti fakat Dataset V2 UUID kolonuna orchestration nesnesi `[object Object]` olarak ulaşınca PostgreSQL `invalid input syntax for type uuid` ile fail-closed oldu. Corrective revizyon Google status endpoint'ini ortak connection contract'ına bağlar ve source job lineage'ını persistence öncesinde scalar UUID'ye normalize edip doğrular. Canlı tekrar kontrolü yapılmadan E5-T7 `Done` sayılmaz; Meta ile aynı completed/no-error kabul sonucu doğrulanmadan E6 başlamaz.
