...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-A+B Done; R6-C prepared, live apply approval pending` — canlı preflight `PASS_PREPARATION_ONLY`; ortak authority→connection→currency→runner→workspace V2 sınırı kodlandı fakat production'a kaydedilmedi. `user_id` nullable aktivasyon migration'ı ve güvenli preflight/postcheck/rollback hazırdır; canlı uygulama ayrı açık onay ister. Gerçek provider kabulü R7-A'nın ilk currency ve canonical connection kaydından sonra R6-D'de yapılır. |
| R7 | Currency-first ve Shopify-native Connect/Disconnect UX | E10-T5 + E10-T6-C2I-V10 | `R7-A blocked by R6-C live apply; R7-B blocked by R6-D` — R7-A Currency→Data Sources→explanation modal→OAuth→verified account→conditional Klaviyo cost foundation'ını açar; R6-D gerçek provider kabulünden sonra R7-B Connected ve warning modal kullanan Disconnect deneyimini tamamlar. |
...
- **R6-C — Repository hazırlığı tamamlandı / canlı migration açık onay kapısı:** Resmî Supabase CLI ile `20260923153220_r6_dataset_v2_workspace_activation.sql` oluşturuldu. Migration yalnız Dataset V2 `user_id` zorunluluğunu kaldırır ve unbound mevcut satır varsa fail-closed olur; `workspace_id NOT NULL`, legacy index/policy retirement, provider aktivasyonu ve veri yazımı yapmaz. Salt-okunur preflight/postcheck ile kontrollü rollback hazırdır. Production'a uygulanmadı; uygulama, postcheck ve advisor kontrolü ayrı açık production onayı gerektirir. R7-A ancak bu canlı şema kapısı PASS olduktan sonra açılır.
