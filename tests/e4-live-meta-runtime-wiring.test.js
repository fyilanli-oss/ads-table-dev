...
test('Meta V2 primary refresh is wired behind one default-off production gate',()=>{assert.match(server,/META_V2_PRIMARY_REFRESH_ENABLED=parseExplicitBoolean\(process\.env\.META_V2_PRIMARY_REFRESH_ENABLED,false/);assert.match(server,/metaV2LiveRefresh=supabaseAdmin\?createMetaLiveRefresh/);assert.match(server,/META_V2_PRIMARY_REFRESH_ENABLED\?metaV2LiveRefresh\.run\(/);assert.match(server,/:writeMetaSnapshotImmutable\(/);});
