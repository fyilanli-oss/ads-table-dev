...
policy_state as (
  ...
  select 'DATASET_V2_BASELINE',(select count(*) from public.performance_dataset_rows_v2),(select dataset_v2_rows from expected)
  union all select 'DATASET_V1_BASELINE',(select count(*) from public.performance_dataset_rows),(select dataset_v1_rows from expected)
  union all select 'SNAPSHOT_BASELINE',(select count(*) from public.dashboard_snapshots),(select snapshot_rows from expected)
  union all select 'E2_T6_RESIDUE',count(*),0 from public.performance_dataset_rows_v2 where raw->>'fixture_namespace' like 'e2\_t6\_rls\_%' escape '\'
