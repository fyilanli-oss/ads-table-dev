...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-D2 Klaviyo live PASS; R6-D3-E Meta data live PASS; R6-D3-F Disconnect contract PASS, implementation next` — Meta legacy kaydı korunmuş ve expired token'ı kullanılmamıştır. Temiz embedded OAuth, bir provider-doğrulanmış hesap seçimi, tamamlanmış E4 motoru ve kontrollü verified-empty Dataset V2 kabulü production'da doğrulanmıştır. Meta'nın bağımsız Disconnect → temiz Reconnect yaşam döngüsü henüz embedded yapıda yoktur; bu kapı tamamlanmadan R6-D3 ve Google Ads R6-D4 açılmaz. |
...
- **R6-D iş aynası — In progress / R6-D1 Done; R6-D2 Klaviyo live PASS; R6-D3-E Meta data live PASS; R6-D3-F Disconnect implementation next:** R6-D tek bir belirsiz production açılışı değildir. Sabit sıra: **R6-D1** ortak fail-closed kabul koşucusu; **R6-D2** Klaviyo workspace tam yaşam döngüsü; **R6-D3** Meta workspace tam yaşam döngüsü; **R6-D4** Google Ads workspace tam yaşam döngüsü; **R6-D5** provider bazlı kontrollü aktivasyon kararı. Her provider Connect → Connected → Disconnect → temiz Reconnect zincirini bağımsız tamamlamadan kendi kabulü kapanmaz. Meta bağlantı/hesap seçimi, E4 Insights/Time/FX ve kontrollü verified-empty Dataset V2 kabulü PASS'tir; Meta Disconnect embedded yapıda eksiktir. TikTok/Pinterest parked kalır. R6-D bütün provider kapıları tamamlanmadan `Done` olmaz; R7-B yalnız son ortak tutarlılık kapısıdır.
...
- **R6-D3-F Meta bağımsız Disconnect yaşam döngüsü — Contract PASS / implementation gate:** Eski `server.js + dashboard.html` Meta Disconnect referansı, yeni embedded kart/route/store yapısı ve canonical Supabase şeması karşılaştırıldı. Embedded Meta yalnız Connected rozeti gösterir; Disconnect modalı, session-bound endpoint ve canonical Meta cleanup operasyonu eksiktir. Mevcut şema `disconnected`, `disconnected_at` ve optimistic `connection_version` ile yeterlidir; migration gerekmez. Meta Disconnect provider revoke başarısından sonra yalnız Meta token/scope/expiry ve seçilmiş hesap alanlarını temizleyecek; Dataset V1/V2, legacy Meta geçmişi, reporting currency, Klaviyo ve gelecekteki Google bağlantısı korunacaktır. Cancel non-destructive, revoke hatası fail-closed, temiz reconnect zorunlu canlı kabul adımıdır. Bu karar paketinde uygulama kodu, provider teması veya Supabase mutation yapılmamıştır. Sözleşme `contracts/r6d3f-meta-independent-disconnect-v1.json`, analist kaydı `docs/R6D3F_META_INDEPENDENT_DISCONNECT.md` içindedir. Sıradaki kapı R6-D3-F uygulamasıdır; Google Ads henüz başlamaz.
