...
| R3 | Dataset V2 workspace tenant dönüşümü | E2 + E3 | `In progress / R3-A+B Done; R3-C1 first-write enforcement prepared` — Dataset V2 canlıda `0` satırdır. Kullanıcı kararıyla `workspace_id NOT NULL`, legacy user index/policy retirement ve browser-direct erişim kapısı ilk Dataset V2 satırından önceye alındı. Production migration henüz uygulanmadı; provider teması ve Dataset write yoktur. |
...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-A+B+C+D1 Done; R6-D2-C1+C2+C3+C4+C5 live PASS; R6-D2-C6-A gate deployed; first write FAILED / rows 0` — C6 ilk canlı deneme HTTP `503` ile fail-closed durdu; Supabase guard okuması başarılı, Dataset V2 upsert oluşmadı ve satır sayısı `0` kaldı. Kullanıcı kararıyla R3-C1 tenant enforcement C6-B teşhisinden ve ikinci denemeden önceye alındı. Tamamlanmış E4/E5/E7 yeniden geliştirilmez. |
...
- **R3 — In progress / R3-A+B Done; R3-C1 first-write enforcement prepared:** R3-A/R3-B workspace authority ve R6-C nullable `user_id` sonucu korunur. C6 ilk canlı write denemesi fail-closed `503` verdi ve Dataset V2 `0` kaldı. Kullanıcı kararıyla final tenant enforcement'ın güvenli alt paketi ilk satırdan önceye alındı: `workspace_id NOT NULL`, workspace FK/unique index korunması, dört legacy user indexinin ve eski authenticated user policy/grant'inin retirement'ı. `user_id` silinmez; nullable non-authoritative actor olarak kalır. Canlı tabloda bağımlı view/function/trigger/cron ve satır yoktur. R3-C1 production migration henüz uygulanmadı; provider teması ve Dataset write yapılmaz. C6-B teşhisi ve ikinci write onayı R3-C1 postcheck PASS sonrasını bekler. Bu pakette provider/OAuth/runtime route/deployment aktive edilmedi.
...
- **R6-D2-C6 kontrollü Dataset V2 canlı kabulü — C6-A deployed / first live attempt FAILED / Dataset V2 rows `0`:** Tek açık onaylı production isteği HTTP `503` ile fail-closed durdu. Shopify authority, canonical connection, reporting currency ve aynı workspace/account/date için Dataset V2 zero-row guard okumaları başarılıydı; Supabase upsert oluşmadı. Bağlantı ve `Placed Order` metric binding korundu. Genel hata maskesi nedeniyle provider/adapter/persistence-preparation aşaması henüz ayrıştırılamaz; tekrar denenmedi. Kullanıcı kararıyla R3-C1 tenant enforcement C6-B güvenli aşama teşhisinden ve ikinci production write onayından önceye alındı. Otomatik retry, cron veya production activation yoktur.
