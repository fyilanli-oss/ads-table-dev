-- E2-T7 corrective diagnostic V2. Read-only; returns gate classifications without counts or identities.
-- Replace exactly the five negative placeholders in an operator-local copy.
with expected(connected_rows,encrypted_rows) as (values ((-71001)::bigint,(-71002)::bigint)), policy_state as (
  ...
  select 'DATASET_V2_BASELINE',(select count(*) from public.performance_dataset_rows_v2),(-71003)::bigint,'eq'
  ...
  union all select 'E2_T4_RESIDUE',(select count(*) from public.performance_dataset_rows_v2 where entity_key='meta:e2_t4_same_key_v2_account:paid:none:campaign:e2_t4_same_key_v2_campaign:ad:e2_t4_same_key_v2_ad'),0,'eq'
  union all select 'E2_T5_RESIDUE',(select count(*) from public.performance_dataset_rows_v2 where entity_key like 'e2\_t5\_rejection\_v1:%' escape '\' or entity_key like 'e2\_t5\_rejection\_v2:%' escape '\'),0,'eq'
  union all select 'E2_T6_RESIDUE',(select count(*) from public.performance_dataset_rows_v2 where (entity_key like 'e2\_t6\_rls\_v1:%' escape '\' or entity_key like 'e2\_t6\_rls\_v2:%' escape '\' or entity_key like 'meta:e2\_t6\_rls\_v3\_%' escape '\' or entity_key like 'meta:e2\_t6\_rls\_v4\_%' escape '\')),0,'eq'
  union all select 'TOTAL_E2_RESIDUE',(select count(*) from public.performance_dataset_rows_v2 where entity_key in ('meta:e2_t3_static_v2_account:paid:none:campaign:e2_t3_static_v2_campaign:ad:e2_t3_static_v2_ad','meta:e2_t4_same_key_v2_account:paid:none:campaign:e2_t4_same_key_v2_campaign:ad:e2_t4_same_key_v2_ad') or (entity_key like 'e2\_t5\_rejection\_v1:%' escape '\' or entity_key like 'e2\_t5\_rejection\_v2:%' escape '\') or (entity_key like 'e2\_t6\_rls\_v1:%' escape '\' or entity_key like 'e2\_t6\_rls\_v2:%' escape '\' or entity_key like 'meta:e2\_t6\_rls\_v3\_%' escape '\' or entity_key like 'meta:e2\_t6\_rls\_v4\_%' escape '\')),0,'eq'
  union all select 'DATASET_V1_BASELINE',(select count(*) from public.performance_dataset_rows),(-71004)::bigint,'eq'
  union all select 'SNAPSHOT_BASELINE',(select count(*) from public.dashboard_snapshots),(-71005)::bigint,'eq'
...
select check_code, case comparison when 'eq' then actual_count=expected_count else true end passed from checks order by check_code;
