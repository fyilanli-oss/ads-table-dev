...
**Durum:** `Verification` — PR #117 merge edildi ve `main` kontrolleri başarılıdır. İlk post-merge deneme `Manual refresh missing Google login customer id` ile job açılmadan durdu. PR #118 hierarchy keşfini, PR #119 doğrudan-root çağrısı ve modal döngüsü düzeltmesini ekledi; canlı tekrar kontrolünde modal döngüsü kapandı fakat discovery hâlâ redacted `Google Ads request failed` verdi. OAuth birden fazla accessible root döndürebildiğinden test-only developer token ile sorgulanamayan tek root tüm keşfi durduruyordu; token alınması da account selection bitmeden sidebar'ı `Connected` gösteriyordu. Corrective revizyon resmi `level <= 1` hierarchy sorgusunu kullanır, root hatalarını izole eder, en az bir başarılı test hierarchy sonucunu korur ve incomplete selection'ı Connected saymaz. Canlı tekrar kontrolü yapılmadan kök neden `Done` sayılmaz; Meta ile aynı completed/no-error kabul sonucu doğrulanmadan E6 başlamaz.
