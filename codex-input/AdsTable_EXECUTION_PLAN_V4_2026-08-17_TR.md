...
- **R6-D4-G Google Sheets + GA4 kontrollü emeklilik ve gerçek Google Ads revoke — Approved / repository implementation:** İki legacy servis tek pakette emekli edilir; çalışma yüzeyleri fail-closed ve durumları `retired` olur. Tarihsel kayıtlar, diğer provider’lar ve reporting currency korunur; migration veya Dataset V2 işi yoktur. Google Ads Disconnect önce ortak Google grant’ini iptal eder; başarısız revoke local bağlantıyı korur. Canlı credential cleanup, Cloud scope/API değişikliği ve temiz reconnect deployment sonrasında ayrı işlem-anı onayı ve postcheck gerektirir. Contract `contracts/r6d4g-google-legacy-retirement-v1.json`, analist kaydı `docs/R6D4G_GOOGLE_LEGACY_RETIREMENT.md` içindedir.
