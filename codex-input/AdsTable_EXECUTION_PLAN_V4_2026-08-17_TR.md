...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-A+B+C+D1 Done; R6-D2-C1 repository corrective prepared, production closed` — PR #250 ile session-bound Klaviyo preflight production'a ulaştı fakat ilk canlı deneme provider çağrısından önce `KLAVIYO_PREFLIGHT_NOT_CONFIGURED` ile durdu; R6-D2 PASS değildir. Global metric env bağı kaldırılarak workspace + Klaviyo account kapsamlı explicit Placed Order metric binding, additive migration ve gizli kabul adımı repository'de hazırlandı. Migration/deployment/provider discovery/connection metadata yazısı/Dataset V2 yazısı yapılmadı. |
...
- **R6-D2-C1 workspace/account metric binding corrective — repository prepared / deployment closed:** Tek global Klaviyo metric ID çok-workspace ve gelecekteki WooCommerce adapter'ı için reddedildi. Metrics API tüm sayfaları provider origin sınırında taranır; yalnız tam `Placed Order` adayları integration name/category provenance ile kabul yüzeyine gelir. Sıfır veya birden fazla aday otomatik seçilmez. Kullanıcının seçimi provider'da tekrar doğrulanarak aynı workspace, canonical Klaviyo account ve optimistic connection version ile `workspace_provider_connections` kaydına bağlanır; disconnect/reconnect/account değişimi bağı temizler. Normal Data Sources yüzeyi değişmez; kontrol yalnız `acceptance=r6d2-klaviyo` altındadır. Focused C1 + Shopify regresyonu 101/101 PASS; tam yerel suite 1002 PASS / 46 FAIL / 1 SKIP olduğu için full-regression PASS iddiası yoktur. Başarısızlıklar tarihsel checksum ve Windows spawn/CRLF gruplarında ayrıştırılmayı bekler. Dataset V2 write bundan sonra da ayrı onaydır.
