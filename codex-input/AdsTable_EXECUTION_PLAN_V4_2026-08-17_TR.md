...
**Durum:** `Verification / Auth gate` — PR #128 modal corrective ve PR #129 sandbox source merge edildi. PR #130 Preview doğrulaması başarısız oldu: ayrı hostname aynı Supabase auth/data plane'ini kullandı, mevcut connection `Disconnect` göründü ve eksik sandbox readiness nedeniyle OAuth fallback yeniden boş advertiser listesi verdi. PR #130 merge edilmeden kapatıldı. PR #132 read-only characterization açık durumdadır. 1 Eylül 2026 insan kararıyla shared Supabase Auth redirect allow-list denetimi ve gerekirse yalnız AdsTable Preview dönüş adresinin eklenmesi onaylandı; bu production deployment/TikTok activation onayı değildir. Management API ağ erişimi sağlanıp mevcut değerler okunmadan yeni kullanıcı retry'ı yapılmaz; sandbox advertiser raporu görülmeden E6-T2/E6-T3 mapping başlamaz.
