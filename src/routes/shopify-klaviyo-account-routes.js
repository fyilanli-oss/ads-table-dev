...
function registerShopifyKlaviyoAccountRoutes(app, {authenticateEmbedded, selection, reset, disconnect, preflight, historicalInventory, datasetAcceptance, metricBinding}) {
  const handler = action => async (req, res) => {
    ...
    try {
      ...
      const known = SAFE_ERRORS.has(error.code) || /^KLAVIYO_DATASET_ACCEPTANCE_FAILED_(CONNECTION|CURRENCY|ACCOUNT_SELECTION|DATASET_GUARD|RUNTIME_CONTEXT|PROVIDER_ACCOUNT|PROVIDER_ACCOUNT_VALIDATION|PROVIDER_FACTS|PROVIDER_RESULT_VALIDATION|FX_RESOLUTION|ROW_NORMALIZATION|RESULT_VERIFICATION|DATASET_PERSISTENCE|PERSISTENCE_CARDINALITY)$/.test(error.code);
