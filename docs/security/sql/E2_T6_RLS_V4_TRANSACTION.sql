-- E2-T6 V4 controlled PostgreSQL-native harness. Execute only after explicit production approval; never remove ROLLBACK.
begin;
...
begin
  ...
  if exists(select 1 from public.performance_dataset_rows_v2 where raw->>'fixture_namespace' like 'e2\_t6\_rls\_%' escape '\') then
...
with inserted as (
  ...
  select internal_user_id,'meta','paid','meta_ads',null,'e2_t6_rls_v4_account_a',current_date,null,
    'campaign','e2_t6_rls_v4_campaign_a','E2 T6 Campaign','adset','e2_t6_rls_v4_adset_a','E2 T6 AdSet',
    'ad','e2_t6_rls_v4_ad_a','E2 T6 Ad','meta:e2_t6_rls_v4_account_a:paid:none:campaign:e2_t6_rls_v4_campaign_a:ad:e2_t6_rls_v4_ad_a',
    ...
    1,1,null,1,0,0,0,0,0,0,'USD','TRY',1,current_date,'e2_t6_fixed_fx','v1','UTC','v1','v1','e2-t6-meta-v4','real',false,null,null,
    '{"fixture_namespace":"e2_t6_rls_v4","actor_label":"user_a"}'::jsonb from pg_temp.e2_t6_rls_actors where actor_label='user_a' returning 1)
...
with inserted as (
  insert into public.performance_dataset_rows_v2 (
    ...
    fx_rate,fx_rate_date,fx_provider,fx_engine_version,source_timezone,time_engine_version,canonical_contract_version,
    adapter_version,source_confidence,synthetic,ga4_property_id,source_job_id,raw)
  select internal_user_id,'meta','paid','meta_ads',null,'e2_t6_rls_v4_account_b',current_date,null,
    'campaign','e2_t6_rls_v4_campaign_b','E2 T6 Campaign','adset','e2_t6_rls_v4_adset_b','E2 T6 AdSet',
    'ad','e2_t6_rls_v4_ad_b','E2 T6 Ad','meta:e2_t6_rls_v4_account_b:paid:none:campaign:e2_t6_rls_v4_campaign_b:ad:e2_t6_rls_v4_ad_b',
    '{"impression":"supported","ad_click":"supported","session":"unsupported","spend_value":"supported","add_to_cart":"supported","add_to_cart_value":"supported","checkout":"supported","checkout_value":"supported","purchase":"supported","purchase_value":"supported"}'::jsonb,
    1,1,null,1,0,0,0,0,0,0,'USD','TRY',1,current_date,'e2_t6_fixed_fx','v1','UTC','v1','v1','e2-t6-meta-v4','real',false,null,null,
    '{"fixture_namespace":"e2_t6_rls_v4","actor_label":"user_b"}'::jsonb from pg_temp.e2_t6_rls_actors where actor_label='user_b' returning 1)
insert into pg_temp.e2_t6_rls_evidence
...
with observed as (select count(*) row_count from public.performance_dataset_rows_v2 where entity_key='meta:e2_t6_rls_v4_account_a:paid:none:campaign:e2_t6_rls_v4_campaign_a:ad:e2_t6_rls_v4_ad_a')
...
with observed as (select count(*) row_count from public.performance_dataset_rows_v2 where entity_key='meta:e2_t6_rls_v4_account_b:paid:none:campaign:e2_t6_rls_v4_campaign_b:ad:e2_t6_rls_v4_ad_b')
...
begin
  begin
    update public.performance_dataset_rows_v2 set entity_name=entity_name where entity_key='meta:e2_t6_rls_v4_account_a:paid:none:campaign:e2_t6_rls_v4_campaign_a:ad:e2_t6_rls_v4_ad_a'; get diagnostics v_count = row_count;
    raise exception using errcode='Z0001',message='local rollback sentinel';
  exception when sqlstate 'Z0001' then null;
    when insufficient_privilege then v_outcome := 'DENIED_BY_PRIVILEGE'; v_count := 0; v_state := sqlstate;
    when others then v_outcome := 'UNEXPECTED_SQLSTATE'; v_count := 0; v_state := sqlstate;
...
begin
  begin
    delete from public.performance_dataset_rows_v2 where entity_key='meta:e2_t6_rls_v4_account_a:paid:none:campaign:e2_t6_rls_v4_campaign_a:ad:e2_t6_rls_v4_ad_a'; get diagnostics v_count = row_count;
    raise exception using errcode='Z0001',message='local rollback sentinel';
  exception when sqlstate 'Z0001' then null;
    when insufficient_privilege then v_outcome := 'DENIED_BY_PRIVILEGE'; v_count := 0; v_state := sqlstate;
    when others then v_outcome := 'UNEXPECTED_SQLSTATE'; v_count := 0; v_state := sqlstate;
...
begin
  begin
    update public.performance_dataset_rows_v2 set entity_name=entity_name where entity_key='meta:e2_t6_rls_v4_account_b:paid:none:campaign:e2_t6_rls_v4_campaign_b:ad:e2_t6_rls_v4_ad_b'; get diagnostics v_count = row_count;
    raise exception using errcode='Z0001',message='local rollback sentinel';
  exception when sqlstate 'Z0001' then null;
    when insufficient_privilege then v_outcome := 'DENIED_BY_PRIVILEGE'; v_count := 0; v_state := sqlstate;
    when others then v_outcome := 'UNEXPECTED_SQLSTATE'; v_count := 0; v_state := sqlstate;
...
begin
  begin
    delete from public.performance_dataset_rows_v2 where entity_key='meta:e2_t6_rls_v4_account_b:paid:none:campaign:e2_t6_rls_v4_campaign_b:ad:e2_t6_rls_v4_ad_b'; get diagnostics v_count = row_count;
    raise exception using errcode='Z0001',message='local rollback sentinel';
  exception when sqlstate 'Z0001' then null;
    when insufficient_privilege then v_outcome := 'DENIED_BY_PRIVILEGE'; v_count := 0; v_state := sqlstate;
    when others then v_outcome := 'UNEXPECTED_SQLSTATE'; v_count := 0; v_state := sqlstate;
...
begin
  begin
    select count(*) into v_count from public.performance_dataset_rows_v2 where entity_key='meta:e2_t6_rls_v4_account_a:paid:none:campaign:e2_t6_rls_v4_campaign_a:ad:e2_t6_rls_v4_ad_a';
...
begin
  begin
    select count(*) into v_count from public.performance_dataset_rows_v2 where entity_key='meta:e2_t6_rls_v4_account_b:paid:none:campaign:e2_t6_rls_v4_campaign_b:ad:e2_t6_rls_v4_ad_b';
  exception when insufficient_privilege then v_outcome := 'DENIED_BY_PRIVILEGE'; v_count := 0; v_state := sqlstate;
    when others then v_outcome := 'UNEXPECTED_SQLSTATE'; v_count := 0; v_state := sqlstate;
  end;
...
), residue as (select greatest(count(*)-2,0) residue_count,count(*) fixture_count from public.performance_dataset_rows_v2 where raw->>'fixture_namespace' like 'e2\_t6\_rls\_%' escape '\'),
parity as (select
...
payload as (select jsonb_build_object(
  'operation_code','e2_t6_rls_v4','expected_case_count',16,'evaluated_case_count',s.evaluated_case_count,
