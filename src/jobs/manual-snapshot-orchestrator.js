...
function createManualSnapshotOrchestrator({ jobBoundary } = {}) {
  ...
  async function run({ userId, platform, platformAccountId, datePreset, snapshotDate, write } = {}) {
    if (typeof write !== "function") throw new TypeError("write must be a function");
    return jobBoundary.run({
      userId,
      platform,
      platformAccountId,
      metadata: { trigger: "manual", datePreset, snapshotDate, captureReason: "manual_refresh", snapshotClass: "primary" },
      work: ({ jobId }) => write(Object.freeze({ sourceJobId: jobId, captureReason: "manual_refresh", snapshotClass: "primary" })),
      completed: (result, job) => ({ snapshot_id: result.snapshot?.id || null, metadata: { ...(job.metadata || {}), performance_spread_result: result.performance_spread_result || null } }),
    });
  }
