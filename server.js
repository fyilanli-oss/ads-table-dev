...
// ===== TEMPORARY GOOGLE TEST / REVIEW CUSTOMER ROUTING =====
// Isolated from OAuth, account selection, snapshots and production behavior.
// Enable only in the Google review/test environment with GOOGLE_TEST_CUSTOMER_ROUTING_ENABLED=1.
const GOOGLE_TEST_CUSTOMER_ROUTING_ENABLED=String(process.env.GOOGLE_TEST_CUSTOMER_ROUTING_ENABLED||"")==="1";
const GOOGLE_TEST_ROUTED_CUSTOMER_ID=process.env.GOOGLE_TEST_ROUTED_CUSTOMER_ID||"5252399301";
const GOOGLE_TEST_ROUTED_LOGIN_CUSTOMER_ID=process.env.GOOGLE_TEST_ROUTED_LOGIN_CUSTOMER_ID||"5383556660";
function resolveGoogleInsightsRouting(requestedCustomerId,requestedLoginCustomerId){
  if(!GOOGLE_TEST_CUSTOMER_ROUTING_ENABLED){
    return {
      customerId:normalizeCustomerId(requestedCustomerId),
      loginCustomerId:normalizeCustomerId(requestedLoginCustomerId||""),
      routingMode:"production"
    };
  }
  return {
    customerId:normalizeCustomerId(GOOGLE_TEST_ROUTED_CUSTOMER_ID),
    loginCustomerId:normalizeCustomerId(GOOGLE_TEST_ROUTED_LOGIN_CUSTOMER_ID),
    routingMode:"test_review"
  };
}
// ===== END TEMPORARY GOOGLE TEST / REVIEW CUSTOMER ROUTING =====
