...
- **E1-T6 — `In progress` — Token protection:** E1-T6A vault hazır, E1-T6B canlı schema/grant acceptance tamamlanmış ve E1-T6C feature-flagged encrypted write/legacy-read artefaktı hazırdır. E1-T6D production dry-run 9 eligible, controlled write-mode ise 7 written/2 failed vermiştir; iki failure aynı auth-orphan kullanıcının Meta/Pinterest bağlantılarıdır. Bu iki kayıt için guarded cleanup artefaktı hazır olsa da canlıda uygulanmamış, production workflow yeniden çalıştırılmamış ve flagler değiştirilmemiştir; `7/7 encrypted, 0 orphan, 0 failure` acceptance sonraki kapıdır.
...
**Gerçekleşen (E1-T6D — `controlled write completed / guarded orphan cleanup pending`):** Production dry-run `9 eligible` verdi. Controlled write-mode `7 written, 2 failed` verdi; iki failure'ın aynı auth-orphan kullanıcıya ait bir Meta ve bir Pinterest bağlantısı olduğu doğrulandı. Canlı acceptance 9 connected, 7 encrypted ve 2 auth-orphan gösterdi. Kullanıcı bu iki orphan bağlantının silinmesini onayladı. Bu PR yalnız fail-closed guarded cleanup migration'ını ve salt-okuma preflight/post-cleanup acceptance artefaktlarını hazırlar; migration canlıda uygulanmadı ve iki bağlantı henüz silinmedi. Production write workflow yeniden çalıştırılmadı, Vercel deploy yapılmadı ve encryption flagleri değiştirilmedi. Mevcut 7 encrypted token kaydı korunmaktadır. E1-T6D `Done` değildir; sonraki kapı migration'ın kullanıcı tarafından ayrıca doğrulanan canlı preflight sonrasında uygulanması ve `7/7 encrypted, 0 orphan, 0 failure` acceptance sonucudur.
...
**Evidence:** E1-T6B schema/grant acceptance tamamlandı. E1-T6D production dry-run 9 eligible; controlled write-mode 7 written ve 2 failed sonucu verdi. İki failure aynı auth-orphan kullanıcının Meta ve Pinterest bağlantıları olarak, canlı acceptance ise 9 connected, 7 encrypted ve 2 auth-orphan olarak doğrulandı. Kullanıcı iki orphan bağlantının silinmesini onayladı. Guarded cleanup migration'ı ve salt-okuma acceptance artefaktları repository içinde hazırlanmıştır ancak migration canlıda uygulanmadı; production write workflow yeniden çalıştırılmadı, Vercel deploy yapılmadı, encryption flagleri değiştirilmedi ve mevcut 7 encrypted token korundu. Evidence token, key, PII veya ham kullanıcı kimliği içermez. E1-T6D `Done` değildir; sonraki kapı doğrulanmış canlı preflight sonrasında kullanıcı uygulaması ve `7/7 encrypted, 0 orphan, 0 failure` kabulüdür.
