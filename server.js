...
async function handleMetaSnapshotWrite(req,res){
  ...
  try{
    ...
    stage="job";
    const execution=await manualSnapshotOrchestrator.run({userId:user.id,platform:"meta",platformAccountId,datePreset,snapshotDate,jobMetadata:{limit,...adminTimeSync,timeEngineVersion:TIME_ENGINE_VERSION},complete:(result,job)=>({snapshot_id:result.snapshot?.id||null,metadata:{...(job.metadata||{}),performance_spread_result:result.performance_spread_result||null,meta_v2_evidence:result.meta_v2_evidence||null}}),write:jobContext=>{stage="meta_api";return META_V2_PRIMARY_REFRESH_ENABLED?metaV2LiveRefresh.run({userId:user.id,accessToken:conn.access_token,accountId:platformAccountId,until:snapshotDate,dayCount:30,sourceJobId:jobContext.sourceJobId,limit:Number(limit)}):writeMetaSnapshotImmutable({user,conn,adAccountId:platformAccountId,datePreset,snapshotDate,limit,platformTimeZone,adminTimeSync,...jobContext})}});
    job=execution.job;
    const writeResult=execution.result;
    stage="snapshot";
