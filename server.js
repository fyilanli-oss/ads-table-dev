...
app.get("/auth/tiktok",async(req,res)=>{try{const accessCheck=await requireConnectAccessForOAuth(req,res);if(!accessCheck)return;const userId=accessCheck.userId;if(!process.env.TIKTOK_CLIENT_ID||!process.env.TIKTOK_CLIENT_SECRET||!process.env.TIKTOK_REDIRECT_URI)throw new Error("Missing TikTok env");const state=Math.random().toString(36).slice(2);req.session.tiktokOAuthState=state;req.session.oauthUserId=userId;const p=new URLSearchParams({client_key:process.env.TIKTOK_CLIENT_ID,response_type:"code",scope:tiktokScopes(),redirect_uri:process.env.TIKTOK_REDIRECT_URI,state});res.redirect(`${TIKTOK_AUTH_BASE}/v2/auth/authorize/?${p}`)}catch(e){res.status(500).send(e.message)}});
app.get("/auth/tiktok/callback",async(req,res)=>{try{const{code,state,error,error_description}=req.query;if(error)return res.redirect(`/dashboard?tiktok_error=${encodeURIComponent(error_description||error)}`);if(!code)return res.redirect("/dashboard?tiktok_error=missing_code");if(!state||state!==req.session.tiktokOAuthState)return res.redirect("/dashboard?tiktok_error=invalid_state");const userId=req.session.oauthUserId;if(!userId)return res.redirect("/dashboard?tiktok_error=missing_user_id");const body=new URLSearchParams({client_key:process.env.TIKTOK_CLIENT_ID,client_secret:process.env.TIKTOK_CLIENT_SECRET,code,grant_type:"authorization_code",redirect_uri:process.env.TIKTOK_REDIRECT_URI});const r=await fetch(`${TIKTOK_API_BASE}/v2/oauth/token/`,{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:body.toString()});const data=await r.json().catch(()=>({}));if(!r.ok||!data.access_token)throw new Error(data.error_description||data.error||data.message||"TikTok token exchange failed");await saveConnection(userId,"tiktok",{accessToken:data.access_token,refreshToken:data.refresh_token||null,tokenExpiresAt:parseExpiry(data.expires_in),metadata:{scope:data.scope||tiktokScopes(),tokenType:data.token_type||null,expiresIn:data.expires_in||null,openId:data.open_id||null}});req.session.tiktokOAuthState=null;res.redirect("/dashboard?tiktok_connected=1")}catch(e){res.redirect(`/dashboard?tiktok_error=${encodeURIComponent(e.message)}`)}});
