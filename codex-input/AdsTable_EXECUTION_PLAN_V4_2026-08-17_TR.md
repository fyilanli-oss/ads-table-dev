...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-A+B+C+D1 Done; R6-D2 live acceptance next` — ortak authority→connection→currency→runner→workspace V2 sınırı ve fail-closed kabul koşucusu hazırdır fakat production composition'a kaydedilmemiştir. R6-C canlı preflight/postcheck ve R6-D1 repository testleri PASS; Dataset V2 `user_id` nullable, veri `0`, `workspace_id` staged nullable ve legacy korumalar yerinde. Klaviyo R6-D2 canlı kabulü için canonical reconnect, read-only preflight ve ayrı açık production provider/write onayı gerekir. |
