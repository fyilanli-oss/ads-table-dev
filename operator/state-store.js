...
function assertOutsideRepo(file, repo) { const relative = path.relative(path.resolve(repo), path.resolve(file)); if (!relative.startsWith('..') || relative === '') throw new Error('Operator state must be outside repository'); }
function writeState(file, state, binding, repo) {
  ...
  const temp = `${file}.${process.pid}.tmp`; fs.writeFileSync(temp, `${JSON.stringify(state)}\n`, { mode: 0o600, flag: 'wx' }); fs.renameSync(temp, file); fs.chmodSync(file, 0o600);
}
function readState(file, binding, repo) {
  assertOutsideRepo(file, repo); let stat; try { stat = fs.statSync(file); } catch { throw new Error('Operator state is missing'); }
  if ((fs.statSync(path.dirname(file)).mode & 0o777) !== 0o700) throw new Error('Operator state directory mode must be 700');
  if ((stat.mode & 0o777) !== 0o600) throw new Error('Operator state mode must be 600');
  let state; try { state = JSON.parse(fs.readFileSync(file, 'utf8')); } catch { throw new Error('Operator state is malformed'); }
