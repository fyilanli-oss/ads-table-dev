...
**C2I-V9 real-device Shopify-native acceptance — FAIL / corrective implemented:** 13 Eylül 2026 gerçek cihaz koşusunda uygulama Shopify Admin içinde açıldı, fakat App Home `SHOPIFY_INSTALL_PERSISTENCE_FAILED` ile durdu. App Home session-first çalışacak şekilde düzeltildi: mevcut active installation doğrudan kullanılır; yalnız `SHOP_REAUTHORIZATION_REQUIRED` durumunda tek bootstrap yazımı ve ardından session doğrulaması yapılır. Ekran görüntüsü ve referans kimliği repository'ye alınmadı. V9 ancak düzeltme deploy edilip App Home yeniden açıldığında tekrar değerlendirilebilir. OAuth, consent, callback, account discovery, provider API veya Production mutation yetkisi verilmez. Ayrıntılı sözleşme `contracts/shopify/e10-t6-c2i-v9-real-device-acceptance.json` ve `docs/E10_T6_C2I_V9_REAL_DEVICE_ACCEPTANCE.md` içindedir.

**C2I-V10 Provider Connect kararı — decision frozen / execution blocked:** Kullanıcı tercihiyle ilk kontrollü provider adayı Klaviyo, fallback Google Ads olarak sabitlendi. Klaviyo akışı `Connect → açıklama modalı → explicit Connect → top-level OAuth → server-verified Klaviyo account seçimi → account-selection close → Email Monthly Plan Cost → Save → Connected` adımlarını ayrı ayrı geçer; OAuth tek başına bağlantı değildir. Disconnect ayrı warning modalı ve açık onay gerektirir, refresh'i durdurur ve tarihsel analytics'i silmez. V9 gerçek cihaz App Home + Data Sources/Platforms insan metin onayı gelmediği için V10 execution kapalıdır; provider consent, API smoke, deployment veya Production mutation yapılmaz. Ayrıntılı sözleşme `contracts/shopify/e10-t6-c2i-v10-provider-connect-decision.json` ve `docs/E10_T6_C2I_V10_PROVIDER_CONNECT_DECISION.md` içindedir.
