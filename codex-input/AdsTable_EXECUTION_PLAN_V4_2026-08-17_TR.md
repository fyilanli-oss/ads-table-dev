...
**Durum:** `Verification / Auth gate` — PR #128 modal corrective ve PR #129 sandbox source merge edildi. PR #130 Preview doğrulaması başarısız oldu: ayrı hostname aynı Supabase auth/data plane'ini kullandı, mevcut connection `Disconnect` göründü ve eksik sandbox readiness nedeniyle OAuth fallback yeniden boş advertiser listesi verdi. PR #130 merge edilmeden kapatıldı. PR #132 read-only characterization açık durumdadır. Supabase URL Configuration ekran kanıtı allow-list'te yalnız production wildcard bulunduğunu ve Preview dönüşünün eksik olduğunu doğruladı; 1 Eylül 2026 insan onayıyla yalnız tam AdsTable Preview `/dashboard` adresinin eklendiği bildirildi. Site URL/production wildcard değişmedi. Şimdi tek kontrollü Preview login doğrulaması beklenir; başarılı olmadan characterization veya E6-T2/E6-T3 mapping başlamaz.
