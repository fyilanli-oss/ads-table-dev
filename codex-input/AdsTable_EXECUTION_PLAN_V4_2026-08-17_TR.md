...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-A+B+C+D1 Done; R6-D2 Klaviyo live PASS; R6-D3-A Meta lifecycle contract Done; R6-D3-B implementation gate` — Meta legacy bağlantısı tarihsel kayıt olarak korunur fakat expired token'ı canonical workspace authority olarak taşınmaz veya kullanılmaz. Temiz embedded OAuth, server-side long-lived token exchange, app/scope/expiry doğrulaması ve 1–3 provider-doğrulanmış hesap seçimi zorunludur; callback tek başına `Connected` değildir. Bu karar provider teması, Dataset V2 yazısı, schedule/backfill veya tamamlanmış E4 işlerinin yeniden geliştirilmesini içermez. |
