...
- **E1-T6 — `In progress` — Token protection:** E1-T6A vault hazır, E1-T6B canlı schema/grant acceptance tamamlanmış ve E1-T6C feature-flagged encrypted write/legacy-read artefaktı hazırdır. E1-T6D production dry-run 9 eligible, controlled write-mode ise 7 written/2 failed vermiştir; iki failure aynı auth-orphan kullanıcının Meta/Pinterest bağlantılarıdır. Bu iki kayıt için schema migration olmayan ve Supabase migration ledger’a dahil edilmeyen guarded operational cleanup SQL hazır olsa da canlıda çalıştırılmamış, production workflow yeniden çalıştırılmamış ve flagler değiştirilmemiştir; doğrulanmış preflight sonrası SQL Editor uygulaması ile `7 connected / 7 encrypted / 0 orphan / 0 missing encrypted` acceptance sonraki kapıdır.
...
**Gerçekleşen (E1-T6D — `controlled write completed / guarded orphan cleanup pending`):** Production dry-run `9 eligible` verdi. Controlled write-mode `7 written, 2 failed` verdi; iki failure'ın aynı auth-orphan kullanıcıya ait bir Meta ve bir Pinterest bağlantısı olduğu doğrulandı. Canlı acceptance 9 connected, 7 encrypted ve 2 auth-orphan gösterdi. Kullanıcı bu iki orphan bağlantının silinmesini onayladı. Bu PR yalnız fail-closed guarded operational cleanup SQL'i ile salt-okuma preflight/post-cleanup acceptance artefaktlarını hazırlar; cleanup SQL canlıda çalıştırılmadı ve iki bağlantı henüz silinmedi. Artefakt schema migration değildir ve Supabase migration ledger'a dahil değildir. Production write workflow yeniden çalıştırılmadı, Vercel deploy yapılmadı ve encryption flagleri değiştirilmedi. Mevcut 7 encrypted token kaydı korunmaktadır. E1-T6D `Done` değildir; sonraki kapı doğrulanmış canlı preflight sonrasında operational cleanup SQL'in kullanıcı tarafından Supabase SQL Editor'da çalıştırılması ve post-cleanup acceptance ile `7 connected / 7 encrypted / 0 orphan / 0 missing encrypted` sonucudur.

**Sonraki kapılar:**

1. **E1-T6B — Schema (acceptance tamamlandı):** İlk ve corrective migration canlıda uygulandı; DDL/constraint/RLS/grant kabulü, forced RLS, istemci grantlerinin yokluğu ve service-role-only CRUD doğrulandı.
...
3. **E1-T6D — Backfill/rotation (`controlled write completed / guarded orphan cleanup pending`):** Dry-run 9 eligible ve controlled write 7 written/2 failed olarak tamamlandı. İki auth-orphan Meta/Pinterest bağlantısı için migration ledger dışında tutulan guarded operational cleanup SQL ve salt-okuma acceptance artefaktı hazırdır; cleanup SQL canlıda çalıştırılmadı, write workflow yeniden çalıştırılmadı ve flag activation yapılmadı.
...
**Evidence:** E1-T6B schema/grant acceptance tamamlandı. E1-T6D production dry-run 9 eligible; controlled write-mode 7 written ve 2 failed sonucu verdi. İki failure aynı auth-orphan kullanıcının Meta ve Pinterest bağlantıları olarak, canlı acceptance ise 9 connected, 7 encrypted ve 2 auth-orphan olarak doğrulandı. Kullanıcı iki orphan bağlantının silinmesini onayladı. Schema migration olmayan guarded operational cleanup SQL ve salt-okuma acceptance artefaktları repository içinde hazırlanmış, Supabase migration ledger'a dahil edilmemiştir; cleanup SQL canlıda çalıştırılmadı; production write workflow yeniden çalıştırılmadı, Vercel deploy yapılmadı, encryption flagleri değiştirilmedi ve mevcut 7 encrypted token korundu. Evidence token, key, PII veya ham kullanıcı kimliği içermez. E1-T6D `Done` değildir; sonraki kapı preflight sonucunun doğrulanması, operational cleanup SQL'in kullanıcı tarafından Supabase SQL Editor'da çalıştırılması ve post-cleanup acceptance ile `7 connected / 7 encrypted / 0 orphan / 0 missing encrypted` kabulüdür.

**E1-T6D production dry-run acceptance ve write artefaktı (2026-08-19):** `production-token-backfill` GitHub Environment oluşturuldu; 3 variable ve 3 secret provision edildi. `main` üzerindeki `f97f1934a98016f129a1bc79263629c2ec8384fa` commit'i için **Provider token production dry-run** run `32245732566` genel, validation ve production dry-run sonuçları success oldu. Redacted sonuç: 9 scanned, 9 eligible, 0 written, 0 already encrypted, 0 rotation candidate, 0 empty, 0 failed ve `nextCursor=null`; dry-run acceptance tamamlandı. Ayrı write operator/CLI/package scripti ve manual protected workflow hazırlandı; immediate preflight, tek encrypted write ve post-write decrypt verification kapıları eklendi. Bu kayıt, write öncesindeki artefakt durumunun tarihsel evidence'ıdır. Daha sonra controlled write-mode çalıştırıldı ve 7 kayıt yazılırken aynı auth-orphan kullanıcıya ait Meta/Pinterest kayıtları olmak üzere 2 kayıt başarısız oldu. Güncel durum `controlled write completed / guarded orphan cleanup pending` olup encryption flagleri değiştirilmedi, Vercel deploy yapılmadı ve E1-T6D `Done` değildir.
