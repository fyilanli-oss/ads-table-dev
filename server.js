...
app.get("/",(_,res)=>sendFile(res,"landing.html")); app.get("/login",(_,res)=>sendFile(res,"login.html")); app.get("/signup",(_,res)=>sendFile(res,"signup.html")); app.get("/dashboard",(_,res)=>sendFile(res,"dashboard.html")); app.get("/demo",(_,res)=>sendFile(res,"dashboard-demo.html")); app.get("/privacy",(_,res)=>sendFile(res,"privacy.html")); app.get("/terms",(_,res)=>sendFile(res,"terms.html")); app.get("/data-deletion",(_,res)=>sendFile(res,"data-deletion.html"));
...
async function requireConnection(req,res,platform){const user=await requireUser(req,res);if(!user)return null;const conn=await getConnection(user.id,platform);if(!conn){res.status(404).json({error:`${platform} not connected`});return null}return{user,conn}}
...
app.get("/api/klaviyo/campaigns",async(req,res)=>{try{const result=await requireConnection(req,res,"klaviyo");if(!result)return;const{conn}=result;const range=String(req.query.date_range||req.query.dateRange||"last_7d");const w=klaviyoDateWindow(range,req.query.start_date,req.query.end_date);const filter=`greater-or-equal(send_time,${w.start}),less-or-equal(send_time,${w.end})`;const data=await klaviyoFetch(conn,`/api/campaigns/?filter=${encodeURIComponent(filter)}`);res.json(data)}catch(e){res.status(500).json({error:e.message})}});
app.get("/api/klaviyo/insights",async(req,res)=>{try{const result=await requireConnection(req,res,"klaviyo");if(!result)return;const{conn}=result;if(conn.metadata?.requiresSetup)return res.status(400).json({error:"Klaviyo setup required. Please enter estimated monthly spend and currency."});const range=String(req.query.date_range||req.query.dateRange||"last_7d");const w=klaviyoDateWindow(range,req.query.start_date,req.query.end_date);const campaignLimit=Math.min(Number(req.query.limit||25),50);const filter=`greater-or-equal(send_time,${w.start}),less-or-equal(send_time,${w.end})`;const campaignsData=await klaviyoFetch(conn,`/api/campaigns/?filter=${encodeURIComponent(filter)}`);const campaigns=(campaignsData.data||[]).slice(0,campaignLimit);let placedOrderMetricId=req.query.placedOrderMetricId||process.env.KLAVIYO_PLACED_ORDER_METRIC_ID||null;if(!placedOrderMetricId)placedOrderMetricId=await getKlaviyoMetricId(conn,["Placed Order","Placed order","Order Placed"]);
...
function googleConversionValue(actions,name){const f=Array.isArray(actions)?actions.find(x=>x.conversionActionName===name||x.conversion_action_name===name||String(x.name||"").endsWith(`/${name}`)):null;return f?Number(f.metrics?.conversions??f.metrics?.allConversions??f.conversions??0):null}
function googleConversionRevenue(actions,name){const f=Array.isArray(actions)?actions.find(x=>x.conversionActionName===name||x.conversion_action_name===name||String(x.name||"").endsWith(`/${name}`)):null;return f?Number(f.metrics?.conversionsValue??f.metrics?.conversions_value??f.conversionsValue??f.conversions_value??0):null}
function normalizeGoogleInsight(row,level){const m=row.metrics||{},c=row.campaign||{},ag=row.adGroup||row.ad_group||{},aga=row.adGroupAd||row.ad_group_ad||{},cust=row.customer||{},seg=row.segments||{},conversionActions=row.conversionActions||row.conversion_actions||[];const spend=microsToMoney(m.costMicros??m.cost_micros),cpc=microsToMoney(m.averageCpc??m.average_cpc),revenue=Number(m.conversionsValue??m.conversions_value??0),conversions=Number(m.conversions??0),invalidClicks=Number(m.invalidClicks??m.invalid_clicks??0),validClicks=Math.max(Number(m.clicks||0)-invalidClicks,0),addToCart=googleConversionValue(conversionActions,"add_to_cart"),checkout=googleConversionValue(conversionActions,"begin_checkout")??googleConversionValue(conversionActions,"checkout"),purchase=googleConversionValue(conversionActions,"purchase")??conversions,purchaseValue=googleConversionRevenue(conversionActions,"purchase")??revenue,abandoned=checkout!==null?Math.max((checkout||0)-(purchase||0),0):null,roas=spend&&spend>0?purchaseValue/spend:null;return{platform:"Google",level,date:seg.date||null,campaign_id:c.id||null,campaign_name:c.name||null,campaign_status:c.status||null,channel_type:c.advertisingChannelType||c.advertising_channel_type||null,bidding_strategy_type:nested(row,"biddingStrategy.type")||nested(row,"bidding_strategy.type")||null,adgroup_id:ag.id||null,adgroup_name:ag.name||null,adgroup_status:ag.status||null,ad_id:nested(aga,"ad.id")||null,ad_status:aga.status||null,currency:cust.currencyCode||cust.currency_code||null,impressions:Number(m.impressions||0),clicks:Number(m.clicks||0),invalid_clicks:invalidClicks,valid_clicks:validClicks,ctr:m.ctr!==undefined?Number(m.ctr)*100:null,cpc,spend,conversions,all_conversions:Number(m.allConversions??m.all_conversions??0),add_to_cart:addToCart,checkout,purchase,purchases:purchase,abandoned,purchase_value:purchaseValue,revenue:purchaseValue,conversion_rate:m.conversionsFromInteractionsRate!==undefined?Number(m.conversionsFromInteractionsRate)*100:m.conversions_from_interactions_rate!==undefined?Number(m.conversions_from_interactions_rate)*100:null,roas,raw:row}}
