...
- **E1-T6 — `In progress` — Token protection:** E1-T6A vault hazır, E1-T6B canlı schema/grant acceptance tamamlanmış, E1-T6C feature-flagged encrypted write/legacy-read artefaktı ve E1-T6D dry-run-only operator harness ile manuel GitHub Actions workflow artefaktı hazırdır. GitHub production environment/secret provisioning yapılmadı, workflow çalıştırılmadı, production key provisioning yapılmadı ve flag kapalıdır; canlı dry-run, ayrı write-mode kararı, plaintext doğrulama ve legacy retirement sonraki kapılardır.
...
**Gerçekleşen (E1-T6D — `artefact ready / GitHub environment provisioning and production dry-run pending`):** Mevcut resumable runner için yalnız dry-run sunan, fail-closed argüman/config doğrulamalı ve allowlist/redacted tek JSON evidence üreten bağımsız operator harness ile ince CLI hazırlandı. Bunları değiştirmeden çağıran, yalnız `workflow_dispatch` ile ve `main` branch guard'ı arkasında çalışan manuel GitHub Actions workflow artefaktı eklendi. Workflow minimum read permission, sabit concurrency, immutable action SHA pinleri, secret içermeyen validation job'ı ve `production-token-backfill` protected environment'ına bağlı production job kullanır; gerekli variable/secret context'leri yalnız operator step'ine scoped edilmiştir. Tracked lockfile ile `npm ci --ignore-scripts --no-audit --no-fund` kurulumu deterministikleştirilmiş, workflow static/security ve regression testleri geçmiştir. Protected environment contract'ı ile manuel işletim adımları runbook'ta belgelendi. Workflow canlıda çalıştırılmadı; GitHub production environment oluşturulmadı, GitHub variable/secret değerleri girilmedi, production key/secret provisioning yapılmadı, canlı dry-run veya write-mode backfill yapılmadı, encryption flag açılmadı ve plaintext tokenlar değiştirilmedi. Bu nedenle E1-T6D `Done` değildir; kullanıcı environment provisioning'i, production dry-run ve sonraki operasyon kapıları beklemektedir.
...
3. **E1-T6D — Backfill/rotation (`artefact ready / GitHub environment provisioning and production dry-run pending`):** Resumable motor, dry-run-only redacted operator harness ve manuel/main-only GitHub Actions workflow artefaktı hazır; GitHub protected environment/secret provisioning, production key provisioning, canlı dry-run, write-mode backfill ve flag activation yapılmadı.
...
**Evidence:** E1-T6B kapsamında ilk ve corrective migration canlıda uygulandı; post-migration grant/RLS acceptance tamamlandı. PR #11 kapsamında canlı dry-run yapılmadı, production key provisioning yapılmadı, encryption flag açılmadı ve write-mode backfill çalıştırılmadı. Sonraki artefakt kapsamında `.github/workflows/provider-token-production-dry-run.yml`, `docs/security/E1_T6D_GITHUB_DRY_RUN_RUNBOOK.md`, tracked `package-lock.json` ve `tests/provider-token-backfill-workflow.test.js` eklendi; workflow tetiklenmedi, GitHub environment/secrets oluşturulmadı ve canlı evidence üretilmedi. Repository evidence'ı schema/operator/workflow static testleri, ilgili hedefli test komutları, temiz lockfile kurulumu sonrası `npm test`, `node --check` ve `git diff --check` ile sınırlıdır; secret, key, token, PII veya kullanıcı kimliği içermez. E1-T6D `Done` değildir ve `artefact ready / GitHub environment provisioning and production dry-run pending` durumundadır.

**E1-T6D production dry-run acceptance ve write artefaktı (2026-08-19):** `production-token-backfill` GitHub Environment oluşturuldu; 3 variable ve 3 secret provision edildi. `main` üzerindeki `f97f1934a98016f129a1bc79263629c2ec8384fa` commit'i için **Provider token production dry-run** run `32245732566` genel, validation ve production dry-run sonuçları success oldu. Redacted sonuç: 9 scanned, 9 eligible, 0 written, 0 already encrypted, 0 rotation candidate, 0 empty, 0 failed ve `nextCursor=null`; dry-run acceptance tamamlandı. Ayrı write operator/CLI/package scripti ve manual protected workflow hazırlandı; immediate preflight, tek encrypted write ve post-write decrypt verification kapıları eklendi. Write workflow canlıda çalıştırılmadı, token yazılmadı, Vercel key provisioning yapılmış sayılmadı, encryption flag açılmadı, plaintext temizlenmedi ve E1-T6D `Done` yapılmadı. Durum: `dry-run accepted / write artefact ready / Vercel key provisioning and controlled write pending`.
