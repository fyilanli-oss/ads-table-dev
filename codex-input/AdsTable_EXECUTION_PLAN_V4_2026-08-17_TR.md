...
**C2I-V9 real-device Shopify-native acceptance — PASS / corrective accepted:** 19 Eylül 2026 ilk gerçek cihaz gözlemi; mükerrer Klaviyo kartı, iki yüzey arasındaki `Connect`/`Connected` çelişkisi, tekrarlanan provider metinleri, aktif görünen Parked Pinterest ve normal render'da provider temas olasılığı nedeniyle `FAIL` oldu. Bounded corrective App Home'u store + Data Sources girişine indirdi, provider metinlerini ayrıştırdı, Pinterest'i `Parked` yaptı ve normal Klaviyo render'ını yalnız yerel status okumasına bağladı. Corrective Production deployment sonrasında 20 Eylül 2026 gerçek cihazda App Home ve Data Sources/Platforms yeniden gözlendi: mükerrer Klaviyo eylemi yoktu; Klaviyo hesap kimliği göstermeden `Connected · 5.00 USD/month` gösterdi ve `Connect` gizlendi; Pinterest `Parked` ve pasifti. İnsan yalnız `Manage data sources` iç navigasyonunu kullandığını, hiçbir Provider Connect düğmesine basmadığını ve OAuth/izin ekranı açılmadığını doğruladı. Ham JPG'ler repository'ye alınmadan mağaza/avatar kimliği maskelendi; iki farklı metadata-free PNG, exact route/file sırası, SHA-256 bağı, güvenli boyutlar ve bütün görsel/privacy/interaction kapıları `artifacts/e10-shopify/e10-t6-c2i-v9/evidence.json` ile fail-closed doğrulandı. Connect modalı, OAuth, account discovery, mutation ve Disconnect V10 kapılarında kalır. Ayrıntılı sözleşme `contracts/shopify/e10-t6-c2i-v9-real-device-acceptance.json` ve `docs/E10_T6_C2I_V9_REAL_DEVICE_ACCEPTANCE.md` içindedir.

**C2I-V10 Provider Connect kararı — decision frozen / V10-A next after V9 evidence merge:** Kullanıcı tercihiyle ilk kontrollü provider adayı Klaviyo, fallback Google Ads olarak sabitlendi. Klaviyo akışı `Connect → açıklama modalı → explicit Connect → top-level OAuth → server-verified Klaviyo account seçimi → account-selection close → Email Monthly Plan Cost → Save → Connected` adımlarını ayrı ayrı geçer; OAuth tek başına bağlantı değildir. Disconnect ayrı warning modalı ve açık onay gerektirir, refresh'i durdurur ve tarihsel analytics'i silmez. V9 gerçek cihaz App Home + Data Sources/Platforms Shopify-native kanıtı `PASS` olmuştur; V10-A Klaviyo açıklama modalı V9 evidence review/merge sonrasındaki sıradaki ayrı pakettir. Bu kayıt kendi başına provider consent, API smoke, deployment veya Production mutation onayı değildir. Ayrıntılı sözleşme `contracts/shopify/e10-t6-c2i-v10-provider-connect-decision.json` ve `docs/E10_T6_C2I_V10_PROVIDER_CONNECT_DECISION.md` içindedir.
