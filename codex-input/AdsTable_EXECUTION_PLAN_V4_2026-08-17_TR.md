...
**2026-09-12 execution-control doğrulaması:** `main` ve açık PR/CI durumu yeniden incelendi; açık PR yoktur ve son `main` Security Regression başarılıdır. Yerel Codex process'inde `VERCEL_TOKEN` görünür değildir; GitHub token'ı Actions secret metadata'sını okumaya da yetkili değildir. Bu iki bulgu Vercel/GitHub secret katmanında token bulunmadığını kanıtlamaz; Vercel GitHub entegrasyonunun Preview deployment'ları başarılıdır. PR #205'in yerel process görünürlüğünü token yokluğu olarak yorumlayan kaydı merge edilmeden kapatılmış, PR #206 net değişiklik bırakmadan geri alınmıştır; dolayısıyla merge düzeltmesi gerektiren açık dal yoktur. E10-T6-C için sıradaki güvenli uygulama sınırı, token değerini hiçbir zaman açığa çıkarmayan ve GitHub `Production` environment korumasında çalışan manual activation workflow'udur. Bu workflow repository'ye alınmadan doğrudan yerel Vercel CLI/API aktivasyonu yapılmaz; production feature flag, deployment ve provider consent kapıları kapalı kalır.
