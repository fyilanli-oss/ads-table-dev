...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-D2 Klaviyo live PASS; R6-D3 Meta full lifecycle live PASS; R6-D4-D Google Ads read-only live PASS; R6-D4-E repository PASS / production acceptance pending` — Google Sheets ve GA4 parked kalır. Kontrollü kabul, yalnız provider-doğrulanmış Google satırlarını canonical Dataset V2 sınırından yazmak üzere hazırlanmıştır; doğrulanmış boş sonuçta sentetik satır üretmez. Canlı provider çağrısı ve Dataset V2 yazısı henüz yapılmamıştır. |
...
- **R6-D4-E Google Ads kontrollü Dataset V2 kabulü — Repository PASS / production acceptance pending:** Tamamlanmış E5 Standard, Performance Max, conversion mapping ve Time/FX motoru ile R6-D4-D workspace runner yeniden kullanılır. Exact action-time confirmation olmadan provider veya Dataset erişimi başlamaz. Seçili 1–3 hesabın `login_customer_id` bağlamı korunur; yakın tarihli mevcut canonical `google` satırı yeniden çalışmayı fail-closed durdurur. Yalnız doğrulanmış satırlar workspace canonical write boundary üzerinden yazılabilir; doğrulanmış boş sonuç `attempted: 0 / persisted: 0` olarak kabul edilir ve sentetik satır üretilmez. Normal Data Sources yüzeyi değişmez; yalnız `?acceptance=r6d4-google` operatör yüzeyi toplu, redacted sonuç gösterir. Schedule, backfill, Dataset V1, Google Sheets, GA4, yeni metrik, Disconnect ve production aktivasyonu kapsam dışıdır. Repository kapsamındaki 9 yeni test ile 76 ilgili regresyon testi PASS'tir. Contract `contracts/r6d4e-google-controlled-dataset-acceptance-v1.json`, analist kaydı `docs/R6D4E_GOOGLE_CONTROLLED_DATASET_ACCEPTANCE.md` içindedir. Canlı provider çağrısı, Supabase yazısı ve production kabulü henüz yapılmadığı için R6-D4-E tamamlanmış sayılmaz.
