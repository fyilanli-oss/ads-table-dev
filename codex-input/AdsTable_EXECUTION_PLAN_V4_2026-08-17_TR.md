...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-A+B+C+D1 Done; R6-D2-C1+C2+C3+C4 live PASS; R6-D2-C5 corrective deployment pending` — C3 exact `Placed Order` discovery ve C4 workspace/account kapsamlı canonical metric binding canlıda doğrulandı. C5 salt-okunur preflight Dataset V2 yazısından önce Klaviyo Campaign Reporting çağrısında kanıtlanmış HTTP `429` ile fail-closed durdu; OAuth, account ve metric binding korunuyor. 429/`Retry-After` düzeltmesi canlı kabul almadan R6-D2 PASS sayılmaz; Dataset V2 yazısı yapılmadı ve tamamlanmış E4/E5/E7 yapıları yeniden geliştirilmez. |
...
- **R6-D2-C5 salt-okunur Klaviyo runtime preflight — Corrective deployment pending:** İlk canlı istek Klaviyo Campaign Reporting endpoint'inde HTTP `429` aldığı için fail-closed `KLAVIYO_PREFLIGHT_FAILED` ile durdu. OAuth/account/metric binding geçerlidir; Dataset V2 yazısı `0` kaldı. Campaign/Flow rapor çağrıları sıralı çalıştırılacak ve 429 yalnız provider `Retry-After` süresi sonrasında sınırlı olarak yeniden denenecek; canlı PASS kanıtı olmadan C5/R6-D2 tamamlanmış sayılmaz.
