...
**Gerçekleşen (E1-T6B — `artefact ready / live acceptance pending`):** Plaintext kolonları genişletmek yerine `platform_connection_tokens` adlı ayrı server-only tablo seçildi. Canlı acceptance'ta `service_role` üzerinde saptanan fazla `REFERENCES`, `TRIGGER` ve `TRUNCATE` yetkilerini önce tüm tablo yetkilerini sıfırlayıp yalnız `SELECT`, `INSERT`, `UPDATE`, `DELETE` veren forward-only `20260819120000_harden_platform_connection_tokens_service_role_grants.sql` corrective migration'ı eklendi. Migration istemci revoke'larını ve forced RLS'i idempotent olarak yeniden uygular; tablo şekline, token satırlarına, owner veya default privilege'lara dokunmaz. Statik regresyon testi least-privilege komut sırasını ve güvenlik sınırlarını denetler. Corrective migration canlıda çalıştırılmadı; uygulama sonrası grant acceptance hâlâ bekliyor ve E1-T6B `Done` değildir.
