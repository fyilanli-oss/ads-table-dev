...
const operation = require('../operator/e2-t6-v2');
...
async function main() {
  ...
  const stateFile = process.env.ADS_TABLE_OPERATOR_STATE
    || path.join(os.homedir(), '.local', 'state', 'ads-table', 'e2-t6-v2.json');
...
if (require.main === module) main().catch((error) => { const safeCode = error && typeof error.safeCode === 'string' ? error.safeCode : 'STOPPED_FAIL_CLOSED'; process.stderr.write(`${JSON.stringify({ operation:'e2_t6_rls_v2', status:'FAIL', safeCode })}\n`); process.exitCode = 1; });
