...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-D2 Klaviyo live PASS; R6-D3-D Meta read-only runtime live PASS; controlled Dataset V2 acceptance pending` — Meta legacy kaydı korunmuş ve expired token'ı kullanılmamıştır. Temiz embedded OAuth, bir provider-doğrulanmış hesap seçimi ve tamamlanmış E4 motorunun workspace authority üzerinden salt-okunur çalışması production'da doğrulanmıştır. Sonuç provider-doğrulanmış boş (`0` satır), Time/FX PASS ve Dataset V2 yazısı `0` olmuştur. Kontrollü Dataset V2 kabulü ayrıca brief/onay/kanıt almadan R6-D3 `Done` değildir. |
