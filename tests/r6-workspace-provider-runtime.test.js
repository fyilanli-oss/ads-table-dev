...
test('R6-D is provider-by-provider and Klaviyo PASS grants no authority to other providers', () => {
  ...
  assert.equal(contract.next_gate, 'R6-D3_META_DATA_RUNTIME_ANALYST_BRIEF');
});
...
test('Execution Plan records the failed C6 attempts and the verified-empty live corrective', () => {
  ...
  assert.match(plan, /R6-D2 Klaviyo live PASS; R6-D3-C Meta connection\/account-selection live PASS; Meta data runtime analysis next/);
