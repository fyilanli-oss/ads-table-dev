...
- **R6-D2-C6 kontrollü Dataset V2 canlı kabulü — C6-A failed closed / C6-B safe-stage diagnostics complete / retry not run / Dataset V2 rows `0`:** Tek açık onaylı production isteği HTTP `503` ile fail-closed durdu. Shopify authority, canonical connection, reporting currency ve aynı workspace/account/date için Dataset V2 zero-row guard okumaları başarılıydı; Supabase upsert oluşmadı. Bağlantı ve `Placed Order` metric binding korundu. C6-B ile connection, currency, Dataset guard, provider account/facts, FX, normalization, verification ve persistence aşamaları allowlist güvenli kodlarla ayrıştırıldı; ham hata, token ve provider payload response/log kapsamına alınmadı. Canlı endpoint tekrar çağrılmadı. Kullanıcı kararıyla R3-C1 tenant enforcement C6-B güvenli aşama teşhisinden ve ikinci production write onayından önceye alındı. Otomatik retry, cron veya production activation yoktur.
