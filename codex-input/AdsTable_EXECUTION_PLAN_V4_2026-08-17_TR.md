...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-D2 Klaviyo live PASS; R6-D3 Meta full lifecycle live PASS; R6-D4 Google Ads analyst brief next` — Meta legacy kaydı korunmuş ve expired token'ı kullanılmamıştır. Temiz embedded OAuth, tek provider-doğrulanmış hesap, tamamlanmış E4 motoru, controlled verified-empty Dataset V2 kabulü ve bağımsız Disconnect → temiz Reconnect yaşam döngüsü production'da doğrulanmıştır. Google Ads kendi tam yaşam döngüsü kabulüne ayrı analist brief'iyle başlar. |
