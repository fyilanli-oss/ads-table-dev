...
function createAutomationSnapshotOrchestrator({ jobBoundary } = {}) {
  ...
  function execute({ userId, platform, platformAccountId, snapshotDate, scheduleId, datePreset, captureReason, snapshotClass, pairedPrimaryJobId = null, extraMetadata = {}, write }) {
  ...
  async function run({ userId, platform, platformAccountId, snapshotDate, scheduleId, policy, primaryMetadata = {}, recoveryMetadata = {}, write } = {}) {
    if (!policy || typeof policy !== "object") throw new TypeError("policy is required");
    if (typeof write !== "function") throw new TypeError("write must be a function");
    const primary = await execute({ userId, platform, platformAccountId, snapshotDate, scheduleId, datePreset: policy.datePreset, captureReason: policy.captureReason, snapshotClass: policy.snapshotClass, extraMetadata: primaryMetadata, write });
    let recoveryResult = null;
    if (policy.shouldRunRecoverySnapshot) {
      try {
        const recovery = await execute({ userId, platform, platformAccountId, snapshotDate, scheduleId, datePreset: policy.recoveryDatePreset, captureReason: policy.recoveryCaptureReason, snapshotClass: policy.recoverySnapshotClass, pairedPrimaryJobId: primary.job.id, extraMetadata: recoveryMetadata, write });
