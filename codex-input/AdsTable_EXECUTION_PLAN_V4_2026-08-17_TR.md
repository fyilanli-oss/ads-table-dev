...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-D2 Klaviyo live PASS; R6-D3-D Meta read-only runtime live PASS; R6-D3-E repository PASS, production controlled acceptance pending` — Meta legacy kaydı korunmuş ve expired token'ı kullanılmamıştır. Temiz embedded OAuth, bir provider-doğrulanmış hesap seçimi ve tamamlanmış E4 motorunun workspace authority üzerinden salt-okunur çalışması production'da doğrulanmıştır. Sonuç provider-doğrulanmış boş (`0` satır), Time/FX PASS ve Dataset V2 yazısı `0` olmuştur. Mevcut workspace writer/repository ile tek kontrollü Dataset V2 kabulü hazırlanmıştır; canlı istek ayrıca açık onay/kanıt almadan R6-D3 `Done` değildir. |
...
- **R6-D iş aynası — In progress / R6-D1 Done; R6-D2 Klaviyo live PASS; R6-D3-D Meta read-only runtime live PASS; R6-D3-E repository PASS / production controlled acceptance gate:** R6-D tek bir belirsiz production açılışı değildir. Sabit sıra: **R6-D1** ortak fail-closed kabul koşucusu; **R6-D2** Klaviyo workspace canlı kabulü; **R6-D3** Meta workspace canlı kabulü; **R6-D4** Google Ads workspace canlı kabulü; **R6-D5** provider bazlı kontrollü aktivasyon kararı. Klaviyo provider-doğrulanmış boş sonuçla PASS olmuştur; bu sonuç Meta veya Google Ads'i aktive etmez. Meta bağlantı/hesap seçimi ile E4 Insights/Time/FX salt-okunur runtime canlıda doğrulanmıştır; Dataset V2 Meta/schedule/job sayıları `0` kalmıştır. Mevcut E4 ve workspace repository'yi kullanan tek kontrollü Dataset V2 kabulü repository'de hazırdır; canlı provider/yazma isteği ayrıca açık onay gerektirir. TikTok/Pinterest parked kalır. R6-D bütün provider kapıları tamamlanmadan `Done` olmaz; R7-B kapalı kalır.
...
- **R6-D3-E Meta kontrollü Dataset V2 kabulü — Repository PASS / production controlled acceptance pending:** Onaylanan analist brief'i doğrultusunda yeni metrik, mapper veya legacy runtime geliştirilmemiştir. Mevcut Meta workspace runner, ortak provider-result doğrulaması, workspace canonical write boundary ve Workspace Supabase repository tek exact-confirmation işleminde birleştirilmiştir. Shopify session dışındaki workspace/tarih/currency alanları authority değildir. Seçilmiş 1–3 hesap için yakın business-date penceresinde mevcut Meta satırı varsa provider temasından önce fail-closed durur. Gerçek satırda `attempted == persisted`; provider-doğrulanmış boş sonuçta `0/0` ve fake-free sonuç zorunludur. Normal Data Sources görünümü değişmez; operatör yüzeyi yalnız `?acceptance=r6d3-meta` altında görünür. Production provider teması, Dataset V2 write, schedule, backfill, V1 write ve aktivasyon yapılmamıştır. Sözleşme `contracts/r6d3e-meta-controlled-dataset-acceptance-v1.json`, analist kaydı `docs/R6D3E_META_CONTROLLED_DATASET_ACCEPTANCE.md` içindedir. Sıradaki kapı production deployment sonrası ayrıca açık onaylı tek kontrollü kabul isteğidir.
