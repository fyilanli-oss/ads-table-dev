...
test('R6 live preflight records preparation-only state without claiming activation', () => {
  assert.equal(contract.status, 'R6_D2_KLAVIYO_LIVE_PASS_R6_D3_META_FULL_LIFECYCLE_LIVE_PASS_R6_D4D_GOOGLE_READ_ONLY_LIVE_PASS_DATASET_ACCEPTANCE_GATE');
...
test('R6-D is provider-by-provider and Klaviyo PASS grants no authority to other providers', () => {
  assert.equal(contract.r6d_work_packages.status, 'R6_D1_COMPLETE_R6_D2_KLAVIYO_LIVE_PASS_R6_D3_META_FULL_LIFECYCLE_LIVE_PASS_R6_D4D_GOOGLE_READ_ONLY_LIVE_PASS_R6_D4E_DATASET_ACCEPTANCE_GATE');
  ...
  assert.equal(contract.next_gate, 'R6-D4-E_GOOGLE_CONTROLLED_DATASET_V2_ACCEPTANCE_BRIEF');
});
...
test('Execution Plan records the failed C6 attempts and the verified-empty live corrective', () => {
  ...
  assert.match(plan, /R6-D2 Klaviyo live PASS; R6-D3 Meta full lifecycle live PASS; R6-D4-C Google Ads connection\/account selection live PASS; R6-D4-D live PASS \/ R6-D4-E next/);
