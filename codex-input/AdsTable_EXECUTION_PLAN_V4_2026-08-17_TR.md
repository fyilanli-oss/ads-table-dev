...
| R6 | Embedded provider runtime → canonical V2 | E4 + E5 + E7 | `In progress / R6-A+B+C+D1 Done; R6-D2-C1 migration + application deployment PASS; R6-D2-C2 next` — PR #250 ile session-bound Klaviyo preflight production'a ulaştı fakat ilk canlı deneme provider çağrısından önce `KLAVIYO_PREFLIGHT_NOT_CONFIGURED` ile durdu; R6-D2 PASS değildir. Global metric env bağı kaldırıldı; workspace + Klaviyo account kapsamlı Placed Order binding migration'ı uygulandı ve PR #251 production'a alındı. Provider discovery, canonical metric yazısı ve Dataset V2 yazısı yapılmadı. C2–C7 tek analist planında donduruldu; tamamlanmış E4/E5/E7 adapter, mapping ve Dataset V2 yapıları yeniden geliştirilmez. |
| R7 | Currency-first ve Shopify-native Connect/Disconnect UX | E10-T5 + E10-T6-C2I-V10 | `R7-A merchant acceptance PASS; R6-D next; R7-B blocked by R6-D` — Klaviyo için currency gate, explanation modal, top-level OAuth, tek verified account, source-currency plan cost, Connected ve revoke-first Disconnect canlıda doğrulandı. Meta/Google 1–3 account sözleşmesi repository ve database sınırında hazırdır; R6-D tamamlanmadan R7-B açılmaz. |
...
- **R6-D iş aynası — In progress / R6-D1 Done; R6-D2-C2 next:** R6-D tek bir belirsiz production açılışı değildir. Sabit sıra: **R6-D1** ortak fail-closed kabul koşucusu; **R6-D2** Klaviyo workspace canlı kabulü; **R6-D3** Meta workspace canlı kabulü; **R6-D4** Google Ads workspace canlı kabulü; **R6-D5** provider bazlı kontrollü aktivasyon kararı. Repository hazırlığı provider teması, deployment veya Dataset V2 yazısı yetkisi vermez. Her canlı kabul kendi canonical `connected` kaydını, merchant-selected reporting currency'yi ve ayrı açık production onayını ister. Klaviyo gerçek non-empty sonuç üretirse yalnız gerçek canonical satırlar yazılır; provider gerçekten boş dönerse `empty_provider_result=true` kanıtlanır ve sentetik satır yazılmaz. Meta/Google 1–3 verified hesap için ayrı ayrı kabul edilir; bir provider PASS sonucu diğerini aktive etmez. TikTok/Pinterest parked kalır. R6-D bütün provider kapıları tamamlanmadan `Done` olmaz; R3-C ve R7-B kapalı kalır.
...
- **R6-D2-C2 Klaviyo satış kaynağı ürün sözleşmesi — Ready / execution yok:** Kullanıcı teknik metric ID seçmez. Tek doğrulanmış `Placed Order` adayı varsa satış kaynağı özeti Email Monthly Plan Cost adımındaki final onayla bir kez doğrulanır; birden fazla doğrulanmış aday varsa yalnız integration/store provenance taşıyan Shopify-native tek seçim kontrolü açılır; sıfır aday fail-closed yeniden yetkilendirme/destek durumudur. Seçim yalnız ilk bağlantı, reconnect/account değişimi, bağın provider'da geçersizleşmesi veya Settings'teki açık değişiklikte yeniden istenir. Plan cost değişikliği tek başına satış kaynağı seçimini tekrar açmaz. Bu paket yalnız analist/UI sözleşmesidir; provider çağrısı, database write, Dataset V2 write veya yeni adapter/mapping geliştirmesi yapmaz.
- **R6-D2-C3 salt-okunur satış kaynağı keşfi — C2 onayı bekler:** Mevcut canonical Klaviyo grant'i ve tamamlanmış Metrics API client'ı ile yalnız exact `Placed Order` adayları bütün sayfalardan okunur; integration name/category provenance dışında ham metric/provider gövdesi UI veya loga taşınmaz. Supabase bağlantı metadata'sı ve Dataset V2 değiştirilmez. Kabul kanıtı aday adedi ve karar durumuyla redacted olur; sıfır/çoklu aday otomatik bağlanmaz.
- **R6-D2-C4 canonical satış kaynağı bağı — C3 PASS + işlem-anı onayı bekler:** C3'te provider tarafından doğrulanan tek/merchant-selected aday, final açık Save onayıyla mevcut `workspace_provider_connections` satırındaki C1 kolonlarına yazılır. Yazma aynı workspace, Klaviyo account ve optimistic connection version ile sınırlandırılır; stale/disconnect/reconnect/account değişimi fail-closed olur. Yeni tablo, adapter, mapping veya Dataset V2 yazısı yoktur.
- **R6-D2-C5 salt-okunur Klaviyo runtime preflight — C4 PASS bekler:** Mevcut R6-D1 koşucusu ve tamamlanmış Klaviyo Accounts + Campaign/Flow Reporting yapısı kullanılır; authority, selected account, source/reporting currency, timezone/business date, bağlı Placed Order kaynağı ve gerçek empty/non-empty sonucu Dataset V2 yazısından önce doğrulanır. Yeni metrik, adapter veya sentetik satır üretilmez; Dataset V2 değiştirilmez.
- **R6-D2-C6 kontrollü Dataset V2 canlı kabulü — C5 PASS + ayrı production write onayı bekler:** Tamamlanmış Klaviyo mapper/runtime ve ortak Dataset V2 writer kullanılır. Provider gerçek non-empty döndürürse yalnız gerçek canonical satırlar yazılır; gerçekten empty döndürürse `empty_provider_result=true` kanıtlanır ve satır uydurulmaz. Yeni veri alanı, mapping, formula veya adapter geliştirilmez.
- **R6-D2-C7 postcheck ve Klaviyo PASS kararı — C6 sonucu bekler:** Workspace/account kapsamı, provenance, currency/FX, duplicate identity, sentetik veri yasağı ve attempted/persisted veya verified-empty sonucu salt okunur kanıtla doğrulanır. Yalnız bütün kontroller PASS ise R6-D2 tamamlanır; aksi durumda fail-closed kalır ve R6-D3'e geçilmez.
