...
Target preflight reads the operator-local `e2_t8.target_kind` and permits only `disposable_supabase` or `official_full_local_supabase`; no kind is hardcoded. Target inventory has exactly the same three-field SHA-256 shape as source inventory. Separate final gates preserve results even for an empty inventory and compute exact aggregate `count(*)` values using only a static, identifier-quoted table allowlist generated from approved inventory; catalog estimates are forbidden. Actual capture, target provisioning, and restore still require separate approval and remain unperformed.
