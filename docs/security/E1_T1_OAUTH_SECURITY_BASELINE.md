...
E1-T5 is complete: production rejects review/sandbox switches, test routes require explicit non-production sandbox mode, and known review identities and query-string sandbox tokens are absent from runtime/UI sources. E1-T6A adds the versioned AES-256-GCM vault; E1-T6B adds the additive forced-RLS token table; E1-T6C adds feature-flagged encrypted writes, explicit legacy reads, fail-closed decrypt and encrypted disconnect cleanup. E1-T6D adds a dry-run-first, bounded and resumable backfill/rotation runner with redacted evidence; scheduled and queued refresh consumers now hydrate through the encrypted connection boundary. Runtime behavior remains unchanged while `PROVIDER_TOKEN_ENCRYPTION_ENABLED` is disabled. Live key provisioning and dry-run acceptance must precede activation.
